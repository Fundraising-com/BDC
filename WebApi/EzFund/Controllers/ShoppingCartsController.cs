    ...
    public class ShoppingCartsController : ApiController
    {
        ...
        public IHttpActionResult Get(int id)
        {
            using (var EZMainUnitOfWork = new UnitOfWork(Database.EZMain))
            {
                ...
                var shoppingCartRepository = EZMainUnitOfWork.CreateRepository<IShoppingCartRepository>();
        ...
        public IHttpActionResult GetByOrderId(int orderId)
        {
            using (var EZMainUnitOfWork = new UnitOfWork(Database.EZMain))
            {
                var productRepository = EZMainUnitOfWork.CreateRepository<IProductRepository>();
                var shoppingCartRepository = EZMainUnitOfWork.CreateRepository<IShoppingCartRepository>();
                var saleRepository = EZMainUnitOfWork.CreateRepository<ISalesRepository>();
                var shoppingCart = shoppingCartRepository.GetByOrderId(orderId);
                var products = shoppingCart.Items.Select(item => item).Where(item => item.ProductId > 0);
                var ezSale = saleRepository.GetEzFundSaleByOrderId(orderId);

                foreach (var product in products) {
                    var subProducts = shoppingCart.Items.Select(item => item).Where(item => item.ParentId == product.Id);
                    var ezItem = new EzFundSaleItem
                    {
                        OrderId = orderId,
                        Product = productRepository.GetSimpleById(product.ProductId),
                        Quantity = product.Quantity
                    };
                    foreach (var subProduct in subProducts)
                    {
                        var ezSubProduct = productRepository.GetSubProductByCode(subProduct.ItemCode);
                        ezSubProduct.SelectedQuantity = subProduct.Quantity;
                        ezItem.Product.SubProducts.Add(ezSubProduct);
                    }
                    ezSale.Items.Add(ezItem);
                }
                return Ok(ezSale);
            }
        }
