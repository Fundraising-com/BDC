...
{	
	public class SampleKit : System.Web.UI.UserControl
	{
		...
		protected System.Web.UI.WebControls.CheckBoxList chkMonthList;
		...
		protected System.Web.UI.WebControls.RadioButtonList rdbDecision;
		...
		protected System.Web.UI.WebControls.Label lblGroupType;
		...
		protected System.Web.UI.WebControls.CustomValidator rfvTitle;
		...
		protected System.Web.UI.WebControls.CustomValidator rfvOrganization;
		protected System.Web.UI.WebControls.CustomValidator rfvGroupType;
		...
		protected System.Web.UI.WebControls.Label lblMonthList;
		...
		{					
			...
			if(!base.IsPostBack) 
			{
				...
				try 
				{
					TitleCollections titleList = TitleCollections.Load();
					if(titleList[__PLEASE_SELECT] == null)
						titleList.Insert(0, new Title(0,__PLEASE_SELECT));
					this.ddlTitle.DataSource = titleList;
					this.ddlTitle.DataBind();
				
					OrganizationCollections orgList = OrganizationCollections.Load();
					if(orgList[__PLEASE_SELECT] == null)
						orgList.Insert(0,new Organization(0,__PLEASE_SELECT));
					
					this.ddlOrgType.DataSource = orgList;
					this.ddlOrgType.DataBind();

                    GA.BDC.Core.efundraisingCore.Geography.StateCollections statesList =
                        GA.BDC.Core.efundraisingCore.Geography.StateCollections.Create("US");
					...
					GroupTypeCollections groupList = GroupTypeCollections.Load();
					if(groupList[__PLEASE_SELECT] == null)
						groupList.Insert(0,new GroupType(0,__PLEASE_SELECT));
					this.ddlGroupType.DataSource = groupList;
					...
					{
						if(countryList[i].CountryCode.ToLower() == "us")
							oCtnIndex = i;
					}
		...
		private void imgSendFreeKit_Click(object sender, System.EventArgs e) 
		{
			...
			if(Session["ADDRESS_VALIDATED"] != null || this.Page.IsValid) 
			{
				...
				int[] oSelectedMonthNumber = new int[12];
				for(int i=0;i<this.chkMonthList.Items.Count;i++) 
				{
					if(this.chkMonthList.Items[i].Selected) 
					{
						oSelectedMonthNumber[i] = i+1;
						oMonth += this.chkMonthList.Items[i].Value + "- ";
					}
				}
				...
				comments += "LONG FORM - " + oProgram + " " + "[Fundraising Date => " + oMonth + "]";
				...
				EfundraisingLead oNewLead = new EfundraisingLead(this.txtFirstName.Text, this.txtLastName.Text, this.txtEmail.Text,
					...
					this.ddlCountry.SelectedValue, this.txtGroupName.Text, this.GetFirstDate(oSelectedMonthNumber).ToString(), 
					this.txtDayPhone.Text, this.txtEvnPhone.Text,this.txtDayPhnExt.Text, this.txtEvnPhnExt.Text, 
					int.Parse(this.ddlOrgType.SelectedValue), byte.Parse(this.ddlGroupType.SelectedValue), 
					int.Parse(this.txtGroupNumber.Text), -1, oEnv.PromotionID, this.ddlTitle.SelectedValue, 
					this.ddlBestTimeToCall.SelectedValue, bool.Parse(this.rdbDecision.SelectedValue), 
					oProgram, this.chkNewsLetter.Checked, comments, 1,0);
