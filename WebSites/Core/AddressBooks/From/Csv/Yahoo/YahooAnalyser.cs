...
namespace GA.BDC.Core.AddressBooks.From.Csv.Yahoo {

	/// <summary>
	/// This is class for parse Yahoo csv based contact list file
	/// </summary>
	internal sealed class YahooAnalyser : CsvAnalyser {
		...
		private string[] _FirstNameField = { "prénom" ,"first", "first name", "prnom","premier", "members" };	
	
		/// <summary>
		/// string array containing somes specifics Lastname columns name
		/// </summary>
		/// <remarks>Should be assignable from configuration settings</remarks>
		private string[] _LastNameField = { "nom", "last name","last", "dernier" };

		/// <summary>
		/// string array containing somes specifics Email columns name
		/// </summary>
		/// <remarks>Should be assignable from configuration settings</remarks>
		private string[] _EmailField = { "email", "courriel" , "adresse électronique", "adresse lectronique" };
		...
		public YahooAnalyser(Stream pStream) {
			...
			for(int i=0;i<oColumnName.Length;i++) {
				if(!oFind) {
					for(int j=0;j<oListHeaderName.Length;j++) 	{
						if(oColumnName[i].ToLower().IndexOf(oListHeaderName[j].ToString(),0,oColumnName[i].Length) != -1) {
							oColumnNumber = i;
							oFind = true;
						}
					}
						...
						while((wLine = usr.ReadLine()) != null) {
							...
							if(this.IsValidEmail(oLine[idxColumnEmail].Trim()) && this.ContactManager.NotInTheList(oLine[idxColumnEmail].Trim())) {
								...
								this.ContactManager.Add(new ContactInfo(GetColumnValue(wLine, idxColumnFirst),
									GetColumnValue(wLine, idxColumnLast), GetColumnValue(wLine, idxColumnEmail)));
								i++;
							}
						}
					} catch(Exception ex) {
