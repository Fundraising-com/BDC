...
namespace GA.BDC.Core.ESubsGlobal.ContentViewer {
	...
	public class Keyword {
		...
		public Keyword() : this(int.MinValue) { }
		public Keyword(int level) : this(level, null) { }
		public Keyword(int level, string key) {
			_level = level;
			_key = key;
		}
		...
		public virtual void Load(XmlNode childNodes) {
			foreach(XmlNode node in childNodes) {
				if(node.Name.ToLower() == "level") {
					SetXmlValue(ref _level, node.InnerText);
				}
				if(node.Name.ToLower() == "key") {
					SetXmlValue(ref _key, node.InnerText);
				}
			}
		}
