(function () {
   ...
   function GoogleAnalyticsService() {
      return {
         SaleCreated: function (sale) {
            ga('set', '&cu', "USD"); // set up currency   

            for (var i = 0; i < sale.Items.length; i++) {
                var item = sale.Items[i];
                for(var x = 0; x < item.Product.SubProducts.length; x++)
                {
                    var subProduct = item.Product.SubProducts[x];
                    if (subProduct.SelectedQuantity > 0) {
                        ga('ec:addProduct', {
                            'id': item.Product.Id,
                            'code':subProduct.ItemCode,
                            'name': subProduct.Name,
                            'category': item.Product.Category.Parent.Name,
                            'brand': item.Product.Category.Name,
                            'price': subProduct.Price,
                            'quantity': subProduct.SelectedQuantity
                        });
                    }
                }
               
            }
            var revenue = sale.TotalAmount - sale.ShippingCalculatedAmount - sale.SaleTaxAmount;
            if (sale.PromotionCode != null && sale.PromotionCode != undefined) {
               revenue += (sale.PromotionCode.DiscountedAmount * -1);
            }
            ga('ec:setAction', 'purchase', {
                'id': sale.OrderId,
               'affiliation': 'EzFund.com',
               'revenue': revenue,
               'tax': sale.SaleTaxAmount,
               'shipping': sale.ShippingCalculatedAmount
            });
         ...
         KitSubmitClicked: function () {
             ga('send', 'event', 'Kit Submit Button', 'Kit button clicked');
         },
         ...
         ProductShowed: function (id, name, category, brand, position, list) {
            ga('ec:addImpression', {
               'id': id,
               'name': name,
               'category': category,
         ...
         ProductClicked: function (id, name, category, brand, position, list) {
   ...
   //function DetectReferralFactory($window) {
   //    return {
   //        Referral: function () {
   ...
   //    };
   //};
   //DetectReferralFactory.$inject = ["$window"];
   //module.factory("DetectReferralFactory", DetectReferralFactory);
