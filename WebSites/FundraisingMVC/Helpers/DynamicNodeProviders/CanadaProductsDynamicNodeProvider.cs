...
namespace GA.BDC.Web.Fundraising.MVC.Helpers.DynamicNodeProviders
{
   ...
   public class CanadaProductsDynamicNodeProvider : DynamicNodeProviderBase
   {
      public override IEnumerable<DynamicNode> GetDynamicNodeCollection(ISiteMapNode node)
      {
         using (var client = new HttpClient())
         {
            ...
            var uri = new Uri(string.Format("{0}/products?rootCategoryId=" + ConfigurationManager.AppSettings["Canada.Root.PackageId"], ConfigurationManager.AppSettings["fundraising.webapi.host"]));
            var products = client
               .GetAsync(uri)
               .Result
