            ...
            Set(ByVal Value As String)
                ...
                If m_strMailFormat = "HTML" Then
                    For x = 0 To Value.Length - 1
                        m_strBody = m_strBody & Chr(Asc(Value.Substring(x, 1)))
                    Next
                Else
                    Dim ar() As String = Split(Value, "&vbcr;")
                    Dim i As Integer
                    Dim NewVal As String = ""
                    For i = 0 To ar.Length - 1
                        NewVal = NewVal & ar(i) & vbCrLf
...
#Region "Function SetErrorMessage(ByVal strMessage As String) As String"
        Protected Function SetErrorMessage(ByVal strMessage As String) As String
            If (m_strError <> "") Then
...
#Region "Sub CDONTS()"
        Private Sub CDONTS()
            ...
            mailer.From = m_strFrom.Trim
            mailer.To = m_strTo.Trim
            mailer.Subject = m_strSubject
            mailer.Body = m_strBody
            If m_strCC <> "" Then
                mailer.Cc = m_strCC.Trim
            End If
            ...
            End If
            mailer.BodyEncoding = mailer.BodyEncoding.UTF8
            SmtpMail.SmtpServer = m_strMailServer
            Try
                '  SmtpMail.Send(mailer.From, mailer.To, mailer.Subject, mailer.Body)
                If mailer.From <> "" And mailer.To <> "" Then
...
#Region "Public Sub SendConfirmationEmail(ByVal m_objOrder As COrder, ByVal m_objCustomer As CCustomer, ByVal m_arEMailContent As ArrayList)"
        Public Sub SendConfirmationEmail(ByVal objOrder As COrder, ByVal objCustomer As CCustomer, ByVal m_arEMailContent As ArrayList)
            m_objOrder = objOrder
            ...
            SendShopperEmail(strOrderTotalContent, strBillingContent, strShippingContent)

            If Not (New CXMLPaymentMethodAccess().GetPaymentMethodName(objOrder.PaymentMethod).ToLower = "phonefax") Then
                SendVendorEmail(strOrderTotalContent, strBillingContent)
            End If
            SendMerchantEmail(strBillingContent, strShippingContent)
        End Sub
#End Region
                ...
                For Each objSortAddress In m_objOrder.OrderAddresses
                    ...
                    While arrVendorIDArrayList.Count > 0
                        ...
                        For i = 0 To arrVendorIDArrayList.Count - 1
                            ...
                            If objVend.VendorID = nCurrVend.VendorID Then
