...
#Region " Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load"
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Me.ErrorMessage.Visible = False
            For Each oItem In tdContent.Controls
                If oItem.GetType.ToString = obj.GetType.ToString Then
                    ...
                    If Product.ISActive Then
                        For Each oItem2 In pnlBot.Controls
                            If oItem2.GetType.ToString = objLinkButton.GetType.ToString Then
                                objLinkButton = CType(oItem2, LinkButton)
                                If objLinkButton.ID = "btnAddToSavedCart" & pnlBot.ID Or objLinkButton.ID = "btnAddToCart" & pnlBot.ID Then
                        ...
                        pnlVolumePriceing = pnlBot.FindControl("pnlVolumePriceing_" & lngProdId)
                        If Me.Product.Inventory.InventoryTracked = False Then
                            If IsNothing(StockInfo) = False Then
                        ...
                        Else
                            ...
                            End If
                        End If
                        If Me.Product.HasVolumePricing = False Then
                            If IsNothing(pnlVolumePriceing) = False Then
                                pnlVolumePriceing.Visible = False
                            End If
                        Else
                            If IsNothing(pnlVolumePriceing) = False Then
                                Volumepricing1 = pnlVolumePriceing.FindControl("Volumepricing1_" & lngProdId)
...
#Region "Private Sub pnlBot_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs) Handles pnlBot.DataBinding"

    Private Sub pnlBot_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs) Handles pnlBot.DataBinding
    End Sub

#End Region

#Region "Public Sub pnlPrice_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)"

    Public Sub pnlPrice_DataBinding(ByVal sender As Object, ByVal e As System.EventArgs)
        pnlPrice = sender
        Dim ar() As String
        Dim sID As String
        Dim sOrigID As String

        ar = Split(CStr(pnlPrice.ID), "_")
        sOrigID = ar(0)
        sID = ar(1)
        ...
        If Not IsNothing(Me.Product) Then
            ...
            CType(pnlPrice.FindControl("lblRegularPriceDisplay_" & sID), Label).Text = PriceDisplay2(tempRegPrice)
            CType(pnlPrice.FindControl("lblSalePriceDisplay_" & sID), Label).Text = "<S>" & PriceDisplay2(tempRegPrice) & "</S>&nbsp;<BR>" & PriceDisplay2(tempSalePrice)
            CType(pnlPrice.FindControl("lblCustomPriceDisplay_" & sID), Label).Text = "<S>" & PriceDisplay2(tempRegPrice) & "</S>&nbsp;<BR>" & PriceDisplay2(tempCustomPrice)
            If Product.IsOnSale And tempSalePrice < tempRegPrice Then
                If tempCustomPrice < tempSalePrice And tempCustomPrice <> 0 Then
                    CType(pnlPrice.FindControl("trCustomPrice_" & sID), HtmlTableRow).Visible = True
                    CType(pnlPrice.FindControl("trSalePrice_" & sID), HtmlTableRow).Visible = False
                    CType(pnlPrice.FindControl("trRegularPrice_" & sID), HtmlTableRow).Visible = False
                Else
                    CType(pnlPrice.FindControl("trCustomPrice_" & sID), HtmlTableRow).Visible = False
                    CType(pnlPrice.FindControl("trSalePrice_" & sID), HtmlTableRow).Visible = True
                    CType(pnlPrice.FindControl("trRegularPrice_" & sID), HtmlTableRow).Visible = False
                End If
            ...
            Else
                CType(pnlPrice.FindControl("trCustomPrice_" & sID), HtmlTableRow).Visible = False
                CType(pnlPrice.FindControl("trSalePrice_" & sID), HtmlTableRow).Visible = False
                CType(pnlPrice.FindControl("trRegularPrice_" & sID), HtmlTableRow).Visible = True
            End If
        End If
