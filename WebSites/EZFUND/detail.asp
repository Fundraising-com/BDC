...
CurrencyISO = getCurrencyISO(Session.LCID )	
   ...
   if Application("AppName")="StoreFrontAE" then	
     ...
     & " INNER JOIN sfSub_Categories ON sfsubCatdetail.subcatCategoryId = sfSub_Categories.subcatID WHERE sfProducts.prodID = '" & makeInputSafe(txtProdId) & "'"
   else
	SQL = "SELECT prodID, prodName, prodShortDescription, prodImageSmallPath, " _
		...
		& "WHERE prodID = '" & makeInputSafe(txtProdId) & "'"
   end if
          ...
          <td width=100%>
           <table border="0" width="100%"> <!--  class="tdContent" -->
						<tr>
							...
							<td align="right"class="tdMiddleTopBanner" width="20%"><a href="<%= C_HomePath %>order.asp"><img src="<%= C_BTN13 %>" border="0" align="right" valign="top" alt="Check Out"></a></td>
						</tr>
	        </table>
	      </td>
        </tr>
