...
Function getProductInfo(sProdID, sCase)
	...
	SQL = "SELECT * FROM sfProducts WHERE prodID = '" & makeInputSafe(sProdID) & "'"
				...
				If Application("AppName")="StoreFrontAE" then
                  SQL = "SELECT subcatCategoryId FROM sfSubCatDetail WHERE prodID = '" & rsProd("prodId") & "'"
     			  ...
     			  iAEcatID = rs("subcatCategoryId")
               	  rs.close
               	   SQL = "SELECT CatHierarchy FROM sfSub_Categories WHERE subcatID = " & makeInputSafe(iAEcatID)
               	   rs.Open SQL, cnn, adOpenForwardOnly, adLockReadOnly, adCmdText
...
Function getAttributeDetails(attrID, iCounter, sCase)
	Set rs = Server.CreateObject("ADODB.Recordset")
	SQL = "SELECT * FROM sfAttributeDetail WHERE attrdtAttributeId = " & makeInputSafe(attrID) & " ORDER BY attrdtOrder"
	rs.Open SQL, cnn, adOpenForwardOnly, adLockReadOnly, adCmdText
...
Dim i
	...
	sql = "Select * FROM sfMTPrices WHERE mtprodid= '" & makeInputSafe(sProdID) & "' ORDER By mtIndex ASC"
	...
	If rst.recordcount > 0 then
		...
		Response.Write " <BR><a href=" & chr(34)  & jsvar & chr(34) & ">Check Volume Discounts</a>"
...
Dim sql, rst
	...
	sql = "Select * FROM sfInventoryInfo WHERE invenProdID= '" & makeInputSafe(strProductID) & "'"
	Set rst = Server.CreateObject("ADODB.RecordSet")		
	rst.Open sql, cnn, adOpenStatic, adLockReadOnly, adCmdText	
...
Dim sql, rst
	...
	sql = "Select Sum(invenInstock) as instock FROM sfInventory WHERE invenProdID= '" & makeInputSafe(strProductID) & "'"
	Set rst = Server.CreateObject("ADODB.RecordSet")		
	rst.Open sql, cnn, adOpenStatic, adLockReadOnly, adCmdText	
	If rst.recordcount <= 0 then 
	...
	If ret = 1 then 
		...
		Select case instock
			...
			case else
				...
				If sType ="Dynamic" then
					Response.Write "<BR><a href=" & chr(34) & jsvar & chr(34) & ">In Stock!</a>"	
				eLse
...
Dim rst,sql
	...
	sql = "Select * FROM sfgiftwraps WHERE gwProdID= '" & makeInputSafe(strProductID) & "'"
	Set rst = Server.CreateObject("ADODB.RecordSet")		
	rst.Open sql, cnn, adOpenStatic, adLockReadOnly, adCmdText	
