...
Select Case sPrefix
	Case "odrattrtmp"
	sLocalSQL = "SELECT odrattrtmpID FROM sfTmpOrderAttributes WHERE odrattrtmpOrderDetailId = " & makeInputSafe(iOrderID)
	Case "odrattrsvd"	
	sLocalSQL = "SELECT odrattrsvdID FROM sfSavedOrderAttributes WHERE odrattrsvdOrderDetailId = " & makeInputSafe(iOrderID)
End Select
...
Dim rsCopy, sLocalSQL, rsSvdCart, rsSvdCartAttr, iKeyID, sReferer,sDateTime,sTmpAttrName, sTmpAttrID, aTmpOrderArray
	...
	sLocalSQL = "Select * FROM sfTmpOrderDetails INNER JOIN sfTmpOrderAttributes ON odrdttmpID = odrattrtmpOrderDetailId "_
				& " WHERE odrdttmpID = " & makeInputSafe(iTmpOrderDetailID) & " And odrdttmpSessionID = " & Session.SessionID
