...
Dim rsDelete, sLocalSQL, rsDelete2, rsDelete3,rsDelete4,sLocalSQL2,sLocalSQL3,sLocalSQL4
	...
	Select Case sPrefix
		Case "odrdttmp"
			sLocalSQL = "DELETE FROM sfTmpOrderDetails WHERE odrdttmpID = " & makeInputSafe(iOrderDetailId)
			sLocalSQL2 = "DELETE FROM sfTmpOrderAttributes WHERE odrattrtmpOrderDetailId = " & makeInputSafe(iOrderDetailId)
			
			If Application("AppName")= "StoreFrontAE" Then
				'Delete records from tmporderdetails ae for this session
				sLocalSQL3 = " Delete FROM sfTmpOrderDetailsAE WHERE odrdttmpAEID = " & makeInputSafe(iOrderDetailId) & ""
				'Move Coupon
		...
		Case "odrdtsvd"
			sLocalSQL = "DELETE FROM sfSavedOrderDetails WHERE odrdtsvdID = " & makeInputSafe(iOrderDetailId)	
			sLocalSQL2 = "DELETE FROM sfSavedOrderAttributes WHERE odrattrsvdOrderDetailId = " & makeInputSafe(iOrderDetailId)
	End Select	
...
Function getTax(choice, sShipping, sTotalPrice, sProdID,isTotal )
	...
	SQL = "SELECT prodCountryTaxIsActive, prodStateTaxIsActive FROM sfProducts WHERE prodID = '" & makeInputSafe(sProdID) & "'"
	...
	Select Case choice
			...
			SQL = "SELECT loclstTax FROM sfLocalesState WHERE loclstAbbreviation = '" & makeInputSafe(sState) & "' AND loclstLocaleIsActive = 1 AND loclstTaxIsActive = 1"
			If vDebug = 1 Then Response.Write SQL & "<br><br>"
			...
			SQL = "SELECT loclctryTax FROM sfLocalesCountry WHERE loclctryAbbreviation = '" & makeInputSafe(sCountry) & "' AND loclctryLocalIsActive = 1 AND loclctryTaxIsActive = 1"
			If vDebug = 1 Then Response.Write SQL & "<br><br>"
			rsTax.Open SQL, cnn, adOpenForwardOnly, adLockReadOnly, adCmdText
			If Not rsTax.EOF Then
				If isTotal = 1 Then	
...
Select Case sPrefix
	Case "odrattrtmp"
		sLocalSQL = "SELECT odrattrtmpAttrID FROM sfTmpOrderAttributes WHERE odrattrtmpOrderDetailId = " & makeInputSafe(sOrderID)
	Case "odrattrsvd"	
		sLocalSQL = "SELECT odrattrsvdAttrID FROM sfSavedOrderAttributes WHERE odrattrsvdOrderDetailId = " & makeInputSafe(sOrderID)
	Case "odr"
		sLocalSQL = "SELECT odrattrID FROM sfOrderAttributes WHERE odrattrOrderDetailId = " & makeInputSafe(sOrderID)
End Select 
...
Dim sLocalSQL, aLocalProdArray(3), rsSelectProd
	...
	sLocalSQL = "SELECT prodName, prodNamePlural, prodPrice, prodAttrNum,prodSaleIsActive,prodSalePrice FROM sfProducts WHERE prodEnabledIsActive=1 AND prodID = '"& makeInputSafe(sProdID) & "'"
...
Dim rsUpdate, sLocalSQL, iOldQuantity, iNewQuantity, rsGetQuantity
	...
	Select Case sPrefix
		Case "odrdttmp"		
				sLocalSQL = "SELECT odrdttmpQuantity FROM sfTmpOrderDetails WHERE odrdttmpID=" &makeInputSafe(iTmpOrderID) & " AND odrdttmpSessionID=" & Session("SessionID")
				If vDebug = 1 Then Response.Write "<br> setUpdateQuantity SQL : " & sLocalSQL

		Case "odrdtsvd"
				sLocalSQL = "SELECT odrdtsvdQuantity FROM sfSavedOrderDetails WHERE odrdtsvdID=" & makeInputSafe(iTmpOrderID) & " AND odrdtsvdCustID=" & Request.Cookies("sfCustomer")("custID") 
	End Select
		...
		Case "odrdttmp"	
			If bHasAttributes Then
				sLocalSQL = "SELECT odrdttmpID, odrattrtmpAttrID FROM sfTmpOrderAttributes INNER JOIN sfTmpOrderDetails ON sfTmpOrderAttributes.odrattrtmpOrderDetailId = sfTmpOrderDetails.odrdttmpID" _
					& " WHERE odrdttmpSessionID = " & Session("SessionID") & " AND odrdttmpProductID = '" & makeInputSafe(sProdID) & "'"
			Else
				sLocalSQL = "SELECT odrdttmpID FROM sfTmpOrderDetails WHERE odrdttmpSessionID = " & Session("SessionID") & " AND odrdttmpProductID = '" & makeInputSafe(sProdID) & "'"
			End If
		Case "odrdtsvd" 
			If bHasAttributes Then
				sLocalSQL = "SELECT odrdtsvdID, odrattrsvdAttrID FROM sfSavedOrderDetails INNER JOIN sfSavedOrderAttributes ON sfSavedOrderDetails.odrdtsvdID = sfSavedOrderAttributes.odrattrsvdOrderDetailId " _
					& " WHERE odrdtsvdCustID=" & Request.Cookies("sfCustomer")("custID") & " AND odrdtsvdProductID = '" & makeInputSafe(sProdID) & "'"
			Else
				sLocalSQL = "SELECT odrdtsvdID FROM sfSavedOrderDetails WHERE odrdtsvdCustID=" & Request.Cookies("sfCustomer")("custID") & " AND odrdtsvdProductID = '" & makeInputSafe(sProdID) & "'"
			End If	
	End Select	
...
Dim sLocalSQL, rsFindAttr, aLocalAttr
	...
	sLocalSQL = "SELECT attrName, attrdtName, attrdtPrice, attrdtType FROM sfAttributeDetail INNER JOIN sfAttributes ON sfAttributes.attrID = sfAttributeDetail.attrdtAttributeId WHERE attrdtID = " & makeInputSafe(iAttrID)
...
Function customerAuth(sEmail,sPassword,sType)
	...
	Select Case sType
		Case "strict"
			sLocalSQL = "SELECT custID FROM sfCustomers WHERE custEmail = '" & makeInputSafe(sEmail) & "' AND custPasswd = '" & makeInputSafe(sPassword) & "' AND custID = " & Session("custID") 
		Case "loose"
			sLocalSQL = "SELECT custID FROM sfCustomers WHERE custEmail = '" & makeInputSafe(sEmail) & "' AND custPasswd = '" & makeInputSafe(sPassword) & "'"
		Case "loosest"
			sLocalSQL = "SELECT custID FROM sfCustomers WHERE custEmail = '" & makeInputSafe(sEmail) & "'" 
		Case else	
			sLocalSQL = "SELECT custID FROM sfCustomers WHERE custEmail = '" & makeInputSafe(sEmail) & "'"
	End Select
...
sSql = "Select slctvalLCID,slctvalCurrencyISO From sfSelectValues Where slctvalLCID = " & "'" &  makeInputSafe(slcid) & "'" 
