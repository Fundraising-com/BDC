...
Partial  Class StandardSearchControl
        ...
        Select Case m_objStorage.SearchingType
            ...
            Case CSearchControlStorage.SearchType.Alpha
                ...
                For Each obj In m_objStorage.DataSource
                    ...
                    ' For Each str In m_objStorage.ColumnList
                    objData = _type.InvokeMember(str, Reflection.BindingFlags.GetProperty, Nothing, obj, Nothing, Nothing)
                    If (objData.ToString().ToLower.IndexOf(m_objStorage.SearchValue.ToLower) = 0) Then
                        ar.Add(obj)
                        'Exit For
                    End If
                    'Next 
            ...
            Case CSearchControlStorage.SearchType.Sort
                ...
                If (IsNothing(Session("LastSearch")) = True) Then
                    Return SortBy(nIndex, m_objStorage.DataSource)
                Else
                    Return SortBy(nIndex, Session("LastSearch"))
                End If
            Case Else
                Return SortBy(0, ar)
        End Select
...
#Region "Function SortBy(ByVal nIndex As Long, ByVal objDataSource As Object) As ArrayList"
    Private Function SortBy(ByVal nIndex As Long, ByVal objDataSource As Object) As ArrayList
...
#Region "Sub BuildAlphabet()"
    Private Sub BuildAlphabet()
        ...
        For Each obj In m_objStorage.DataSource
            ...
            Else
                Try
                    objCol.Remove(objData.ToString().Substring(0, 1).ToUpper())
                    objCol.Add("1", objData.ToString().Substring(0, 1).ToUpper())
    ...
    Public Property StorageClass() As CSearchControlStorage
        Get
            If (IsNothing(m_objStorage) = True) Then
                m_objStorage = Session("SearchStorage")
            End If
            Return m_objStorage
        End Get
        Set(ByVal Value As CSearchControlStorage)
            ...
            Else
                ...
                End If
            End If
            Session("SearchStorage") = m_objStorage
...
#Region "Edit Item"
    ...
    Public Sub EditItem(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim objData As Object
        DataGrid2.DataSource = DoSearching()
        Dim _type As Type = DataGrid2.DataSource.Item(CLng(sender.CommandName) + (DataGrid2.PageSize * DataGrid2.CurrentPageIndex)).GetType()
        objData = _type.InvokeMember(m_objStorage.ButtonID, Reflection.BindingFlags.GetProperty, Nothing, DataGrid2.DataSource.Item(CLng(sender.CommandName) + (DataGrid2.PageSize * DataGrid2.CurrentPageIndex)), Nothing, Nothing)

        RaiseEvent EditClick(objData, EventArgs.Empty)
    End Sub
#End Region
...
#Region "Delete Item"
    ...
    Public Sub DeleteItem(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Try
            ...
            RaiseEvent DeleteClick(objData, EventArgs.Empty)
        Catch objErr As Exception
    ...
    Private Sub DataGrid2_ItemCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles DataGrid2.ItemCreated
        ...
        ElseIf (e.Item.ItemType = ListItemType.AlternatingItem Or e.Item.ItemType = ListItemType.Item) Then
            If (IsNothing(e.Item.DataItem) = False) Then
                ...
                If (m_objStorage.CheckBoxColumn = False) Then
                    e.Item.FindControl("chkCell").Visible = False
                Else
                    ...
                    e.Item.FindControl("chk").ID = "chk" & objData
                End If
                ...
                If (m_objStorage.ShowButtons = False) Then
                    e.Item.FindControl("btnCell").Visible = False
                Else
                    Dim objButton As LinkButton
                    objData = _type.InvokeMember(m_objStorage.ButtonID, Reflection.BindingFlags.GetProperty, Nothing, e.Item.DataItem, Nothing, Nothing)

                    objbutton = e.Item.FindControl("btnEdit")
                    If IsNothing(objButton) = False Then
                        objbutton.CommandArgument = objData
                    End If
                    ...
                    If IsNothing(objButton) = False Then
                        objbutton.CommandArgument = objData
                        objButton.Attributes.Add("onclick", "javascript:return ConfirmCancel('" & m_objStorage.DeleteMessage & "');")
                    End If
                End If
            End If
        End If
...
#Region "Overrides Function OnBubbleEvent(ByVal source As Object, ByVal args As System.EventArgs) As Boolean"
    Protected Overrides Function OnBubbleEvent(ByVal source As Object, ByVal args As System.EventArgs) As Boolean
        ...
        If (source.GetType Is GetType(LinkButton)) Then
            ...
            '    tblSearch.Visible = True
            'End If
            'Session("LastSearch") = DataGrid2.DataSource
        End If
    End Function
#End Region
    ...
    Private Sub DataGrid2_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles DataGrid2.ItemCommand
        If (e.CommandSource.GetType Is GetType(LinkButton)) Then
            ...
            ElseIf (objButton.Text.IndexOf("Previous") <> -1) Then
                DataGrid2.CurrentPageIndex = CType(source, DataGrid).CurrentPageIndex - 1
            End If
            StorageClass = Session("SearchStorage")
            DataGrid2.DataSource = DoSearching()
            DataGrid2.DataBind()
        End If
    End Sub
