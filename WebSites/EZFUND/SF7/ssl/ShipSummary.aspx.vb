...
Partial Class ShipSummary
    ...
    'sp7
    Protected WithEvents btnPayPalShipToEdit As System.Web.UI.WebControls.LinkButton
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            Dim PayPalError As String
            PayPalError = IIf(IsNothing(Request.QueryString("PayPalError")), False, True)
            If objOrder.IsPayPalOrder AndAlso PayPalError.ToLower = "false" Then
                DataList1.Items(0).FindControl("btnPayPalShipToEdit").Visible = True
                CType(DataList1.Items(0).FindControl("btnPayPalShipToEdit"), LinkButton).DataBind()
                TotalDisplay1.isPayPalOrder = True
            Else
                DataList1.Items(0).FindControl("btnPayPalShipToEdit").Visible = False
                TotalDisplay1.isPayPalOrder = False
                ErrorMessage.Visible = True
                ErrorMessage.Text = Session("PayPalError")
                Session("PayPalError") = Nothing
            End If
            'sp7
            ...
            For Each item In DataList1.Items
                ...
                objAddress = CType(objOrder.OrderAddresses.Item(item.ItemIndex), COrderAddress) '2219
    ...
    Private Sub DataList1_ItemCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListItemEventArgs) Handles DataList1.ItemCreated
        'Tee 10/9/2007 check ensure customer still in valid session
        If (Session.IsNewSession AndAlso Not IsNothing(Request.Headers("Cookie")) AndAlso Request.Headers("Cookie").IndexOf("ASP.NET_SessionId") >= 0) Then
            Response.Expires = 0
    ...
    Private Sub btnContinue_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnContinue.Click
        ...
        For Each objItem In DataList1.Items
            ...
            For Each _orderaddress In objOrder.OrderAddresses
                If (_orderaddress.Address.ID = txtShipID.Text) Then
                    ...
                    If objShip.SelectedItem.Value <> "NONE" Then '2393
        ...
        If IsNothing(CType(TotalDisplay1.FindControl("PaymentMethod"), SelectValControl)) = False AndAlso CType(TotalDisplay1.FindControl("PaymentMethod"), SelectValControl).SelectedItem.Text.ToLower = "paypal" Then
        ...
        If strpaymentmethod.Equals("paypal") Then
            If objOrder.OrderTotal <= 0 Then
                ErrorMessage.Visible = True
                ErrorMessage.Text = "PayPal does not accept Order Total less than 0. <br> Please select a different form of payment"
                Exit Sub
            ElseIf objOrder.OrderAddresses.Count > 1 Then
                ErrorMessage.Visible = True
                ErrorMessage.Text = "PayPal does not accept payment for Orders shipping to multiple addresses. <br> Please select a different form of payment"
                Exit Sub
            Else
                Server.Transfer("HandlePayPalExpress.aspx?WebID=" & strWeb & "&From=SS&Method=paypal")
            End If
        ElseIf objOrder.IsPayPalOrder Then
            If objOrder.OrderTotal <= 0 Then
                ErrorMessage.Visible = True
                ErrorMessage.Text = "PayPal does not accept Order Total less than 0.Please select a different form of payment"
                Exit Sub
            Else
                Server.Transfer("HandlePayPalExpress.aspx?WebID=" & strWeb & "&From=SS", True)
            End If
        ElseIf objOrder.BillAddress Is Nothing Then
            Response.Redirect("Billing.aspx?WebID=" & strWeb)
        Else
    ...
    ' -----------------------------------------------------------------------------
    ' <summary>Public Sub EditPayPalShipTo
    ' </summary>
    ' <remarks>Handles EditPayPalShipTo click. This would redirect them to PayPal in 
    ' order to allow them to edit the paypal Ship To Address
    ' </remarks>
    ' <history>AB Code 
    ...
    ' </history>
    ' -----------------------------------------------------------------------------
    Public Sub EditPayPalShipTo(ByVal sender As Object, ByVal e As System.EventArgs)
        Server.Transfer("HandlePayPalExpress.aspx?WebID=" & m_objcustomer.GetSessionID & "&From=SS&Edit=True", True)
    End Sub
    'End SP7
End Class
