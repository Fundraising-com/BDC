...
#Region "Class Members"
    ...
    Protected WithEvents trSubscriptionPrice As HtmlTableRow
    Protected WithEvents trRecurringPrice As HtmlTableRow
    'Verisign Recurring Billing
    ...
    'OSP
    Protected WithEvents trAddToCart As HtmlTableRow
    ...
    'BEGIN: GJV - 8/23/2007 - OSP merge
	'OSP
	Protected WithEvents trOptions As HtmlTableRow
    Protected WithEvents line1 As HtmlTableRow
    Protected WithEvents line2 As HtmlTableRow
    Protected WithEvents line3 As HtmlTableRow
    Protected WithEvents line4 As HtmlTableRow
    Protected WithEvents line5 As HtmlTableRow
    'END: GJV - 8/23/2007 - OSP merge
    ...
    'BEGIN: GJV - 8/23/2007 - OSP merge
	'OSP
	Private m_AddToCart As Boolean
    'END: GJV - 8/23/2007 - OSP merge
    'Tee 10/23/2007 added member
    Private trStockStatus As HtmlTableRow
    'end Tee
#End Region
...
#Region "Sub LoadSettings(ByVal objDetail As XmlNode)"
    Public Sub LoadSettings(ByVal objDetail As XmlNode)
        ...
        'BEGIN: GJV - 8/23/2007 - OSP merge
		'OSP
		 m_AddToCart = IIf(objDetail.Attributes("AddToCart").Value = "1", True, False)
		 'END: GJV - 8/23/2007 - OSP merge
    End Sub
#End Region
...
#Region "Sub SetDisplay1()"
    Public Sub SetDisplay1()
        ...
        'Verisign Recurring Billing
        trSubscriptionPrice.Visible = m_bDisplayPriceSalePrice
        trRecurringPrice.Visible = m_bDisplayPriceSalePrice
        'Verisign Recurring Billing
        ...
        'BEGIN: GJV - 8/23/2007 - OSP merge
        'SFExpress
        If Not IsNothing(trAddToCart) Then
            trAddToCart.Visible = m_AddToCart
            If trAddToCart.Visible = False AndAlso trSavedCart.Visible = False Then
                ...
                line5.Visible = False
            End If
            If Not m_bDisplayPriceSalePrice Then
                trPriceSpacer.Visible = False
                trPrice.Visible = False
                trCustomPrice.Visible = False
                trSavings.Visible = False
                trSalePrice.Visible = False
                trVolumePricing.Visible = False
            End If
        End If
        'END: GJV - 8/23/2007 - OSP merge
        'Verisign Recurring Billing
        If Product.ProductType = ProductType.Subscription OrElse _
                Product.ProductType = ProductType.BundleSubscription OrElse _
                Product.ProductType = ProductType.CustomizedSubscription Then
            If trPrice.Visible = True Then
                trPrice.Visible = False
                trSubscriptionPrice.Visible = True
            Else
                trSubscriptionPrice.Visible = False
            End If
            trRecurringPrice.Visible = True
        Else
            trSubscriptionPrice.Visible = False
            trRecurringPrice.Visible = False
        End If
        'Verisign Recurring Billing
    End Sub
#End Region

#Region "Public Sub SetDisplay2(Optional ByVal DYOMDetail As Boolean = False)"
    Public Sub SetDisplay2(Optional ByVal DYOMDetail As Boolean = False)
        Dim CustomPrice As String
        trProductName.Visible = m_bDisplayProductName
        trProductCode.Visible = m_bDisplayProductCode
        trVendor.Visible = m_bDisplayVendor
        trManufacturer.Visible = m_bDisplayManufacturer
        If Not DYOMDetail Then
            trPrice.Visible = m_bDisplayPriceSalePrice
            'Verisign Recurring Billing
            trSubscriptionPrice.Visible = m_bDisplayPriceSalePrice
            trRecurringPrice.Visible = m_bDisplayPriceSalePrice
...
#Region "Function SetCustomPriceValue(ByVal Id As Long) As String"
    Private Function SetCustomPriceValue(ByVal Id As Long) As String
        ...
        If m_objProduct.IsOnSale Then
            If CustPrice >= m_objProduct.SalePrice Then
                'Tee 7/16/2007 check sale type
                If m_objProduct.SaleType = 0 Then
                ...
                ElseIf m_objProduct.SaleType = 1 Then
                    Return StoreFrontConfiguration.Labels.Item("lblPrice").InnerText.Trim & ": <s>" & PriceDisplay2(m_objProduct.Price) & "</s><BR>" & StoreFrontConfiguration.Labels.Item("lblClearancePrice").InnerText().Trim() & ": " & PriceDisplay2(m_objProduct.SalePrice)
                End If
                'end Tee
            Else
            ...
            If Product.Inventory.InventoryTracked = True Then
                If Product.Inventory.ShowStatus = True Then
                    ...
                    Else
                        ...
                        StockInfo.Attributes.Add("OnClick", "return DisplayStatus('" & StockInfo.ClientID & "');")
                        'end Tee
                    End If
                Else
...
#Region "Private Function arRelatedProd(Optional ByVal startIndex As Long = 0, Optional ByVal endIndex As Long = 0) As ArrayList"

    Private Function arRelatedProd(Optional ByVal startIndex As Long = 0, Optional ByVal endIndex As Long = 0) As ArrayList
        ...
        Else
            ...
            'Tee 10/22/2007 related products
            If startIndex > 0 OrElse endIndex > 0 Then
                Dim arrRelated As New ArrayList
                For i As Integer = startIndex To CInt(IIf(endIndex > Product.RelatedProducts.Count - 1, Product.RelatedProducts.Count - 1, endIndex))
                    arrRelated.Add(Product.RelatedProducts(i))
                Next
                ds = oprodManagement.RelatedItemsDisplay(arrRelated, m_objCustomer.CustomerGroup)
            Else
...
#End Region
    ...
    'Tee 10/22/2007 related product in datalist
#Region "Public Sub RelatedProducts(ByVal dlRelatedProducts As DataList, ByVal startIndex As Long, ByVal endIndex As Long)"
    Public Sub RelatedProducts(ByVal dlRelatedProducts As DataList, ByVal startIndex As Long, ByVal endIndex As Long)
        If IsNothing(Product) Then
            Product = Session("dProduct")
        End If
        If (Product.RelatedProducts.Count > 0 AndAlso IIf(StoreFrontConfiguration.ProductDetail.Attributes("DisplayRecommendedProducts").Value = "1", True, False)) Then
            dlRelatedProducts.DataSource = arRelatedProd(startIndex, endIndex)
            dlRelatedProducts.DataBind()
        Else
            dlRelatedProducts.Visible = False
        End If
    End Sub
#End Region
    'end Tee
...
#Region " Public Sub StockButton_Click(ByVal sender As Object, ByVal e As System.EventArgs)"
    ...
    Public Sub StockButton_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If IsNothing(Product) OrElse Product.ProductID <> CType(Session("dProduct"), CCartItem).ProductID Then
            Product = Session("dProduct")
        End If
