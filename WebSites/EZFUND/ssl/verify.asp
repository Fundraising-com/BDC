...
CurrencyISO = getCurrencyISO(Session.LCID )	
    ...
    Dim ST_TaxableAmount 'State
	Dim CT_TaxableAmount 'country
        bFromVerify = false
...
<table border="0" cellpadding="1" cellspacing="0" class="tdbackgrnd" width="<%= C_WIDTH %>" align="center">
  <tr>
    ...
    Please review all your information as listed below. Once the <b>Checkout</b> button is pressed, your transaction will be final.
        </tr>
        <tr>
          <td width="100%" align="center" class="tdContent2"  valign="center">Step 1: Customer Information | <b>Step 2: Payment Information</b> | Step 3: Complete Order</td>
        </tr>
        <tr>
...
'				ElseIf Trim(iShip) = 0 Then
		...
		iSTax = cDbl(getTax("State", sShipping, dProductSubtotal, sProdID,0))
		If iStax <> 0 then
			ST_TaxableAmount = ST_TaxableAmount + dProductSubtotal
		End if
	
	
		iCTax =  cDbl(getTax("Country", sShipping, dProductSubtotal, sProdID,0))
        If iCTax <> 0 then
			CT_TaxableAmount = CT_TaxableAmount + dProductSubtotal
		End if
        'End #568  
	...
	If Application("AppName") = "StoreFront" Then 'SFUPDATE
		...
		ST_TaxableAmount = getGlobalSalePrice(ST_TaxableAmount) '#663
		CT_TaxableAmount = getGlobalSalePrice(CT_TaxableAmount) '#663
	End If
	
	IF Application("AppName") = "StoreFrontAE" Then 'SFAE
		sTotalPrice =  ApplyALLDiscounts(cdbl(sTotalPrice),"Total") '.3008
		ST_TaxableAmount =  ApplyALLDiscounts(cdbl(ST_TaxableAmount),"Total") '#663
		CT_TaxableAmount =  ApplyALLDiscounts(cdbl(CT_TaxableAmount),"Total")  '#663
		SetBillingVariables
	...
	'#568
	iSTax = cDbl(getTax("State", sShipping, ST_TaxableAmount, sProdID,1))
	iCTax =  cDbl(getTax("Country", sShipping, CT_TaxableAmount, sProdID,1))
...
'		          End if 
	...
	<% 	If iShipType = 2 Then %>	          
		          ...
		          <td width="80%" colspan="4"><table><tr>
		           <td><% If InStr(sShipMethodName,"UPS") Then %><img src="images/logo_s.gif"><% Else %>&nbsp;<% End If %></td><td><font class="Content_Small">
NOTICE: The shipping costs as shown above&nbsp;do not necessarily represent the carrier's 
published rates&nbsp;and may include additional charges levied by the merchant.</font></td></tr>
</table ></td>
</tr>
<% End If %>
		        </table>
           ...
           <td width="100%" class="tdContent2"> 
                ...
                <td width="50%"><b>Card Number<font color="#FF0000">*</font>:</b></td>
			        ...
			        <tr>
			          ...
			          <td width="50%">Month <input type="text" size="2" name="CardExpiryMonth" title="Credit Card Month" onfocusout="return ValidateMe(this)" Style="<%= C_FORMDESIGN%>">   
			             Year <input type="text" size="4" name="CardExpiryYear" title="Credit Card Year" onfocusout="return ValidateMe(this)" Style="<%= C_FORMDESIGN%>"></td>
			        </tr>
