		   ...
		   If iTotalPur > 0 Then
			     	'TotaL_with_Attributes = iTotalPur 'Upadating fix to #286
						...
						Do While NOT rsShipping.EOF
						   ...
						   End If			
							If iQuantity > 0 Then 
						    	SQL = "SELECT ProdPrice,ProdSalePrice,ProdSaleIsActive,ProdShipIsActive FROM sfProducts WHERE prodShipIsActive = 1 AND prodID = '" & makeInputSafe(sProdID) & "'"
...
'-----------------------------------------------------------------			
		...
		Case 2 
			...
			If sShipping <> "FAIL" Then	
				...
				SQL = "SELECT shipMethod,shipCode, shipRates FROM sfShipping WHERE shipID = " & makeInputSafe(sShipCode)
				If vDebug = 1 Then Response.Write SQL & "<br><br>"
				rsShipping.Open SQL, cnn, adOpenForwardOnly, adLockReadOnly, adCmdText
...
'-jf 8/23/01
				...
				ElseIf Instr(rsShipping.Fields("shipCode"), "USPS") <> 0 Then
						...
						ElseIf rsShipping.Fields("shipCode") = "USPSInternational" Then
						  SQL = "SELECT loclctryName FROM sfLocalesCountry WHERE loclctryAbbreviation = '" & makeInputSafe(dCountry) & "'"
			...
			Set rsProdShipping = Server.CreateObject("ADODB.RecordSet")
				...
				Do While NOT rsShipping.EOF
				   ...
				   End If			
					...
					SQL = "SELECT prodShip FROM sfProducts WHERE prodShipIsActive = 1 AND prodID = '" & makeInputSafe(sProdID) & "'"
					rsProdShipping.Open SQL, cnn, adOpenForwardOnly, adLockReadOnly, adCmdText 

					'Response.Write CDbl(iQuantity)
					If Not rsProdShipping.EOF Then 
...
tmpAmt = iTotalPrice
   ...
   sql = "SELECT * from sfTmpOrderAttributes where odrattrtmpOrderDetailID = " & makeInputSafe(DetID)
      rsProdShipping.Open sql, Cnn, adOpenForwardOnly, adLockReadOnly, adCmdText
...
Function getPassword(iCustID)
	...
	If iCustID <> "" Then
		Set rsPass = Server.CreateObject("ADODB.Recordset")
			sSQL = "SELECT custPasswd FROM sfCustomers WHERE custID = " & makeInputSafe(iCustID)
...
Sub setUpdateSvdCustomer(sNewEmail,sPassword,sFirstName,sMiddleInitial,sLastName,sCompany,sAddress1,sAddress2,sCity,sState,sZip,sCountry,sPhone,sFax,bSubscribed,iCustID)
	...
	sLocalSQL = "Select custFirstName, custMiddleInitial, custLastName, custCompany, custAddr1, custAddr2, custCity, custState, custZip, custCountry, "_
				& "custPhone, custFax, custTimesAccessed, custLastAccess, custEmail, custPasswd, custIsSubscribed FROM sfCustomers WHERE custID = " & makeInputSafe(iCustID)
