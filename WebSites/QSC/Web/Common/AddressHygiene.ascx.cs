namespace QSPFulfillment.CommonWeb.UC
{
	...
	/// </remarks>
	public partial class AddressHygiene : System.Web.UI.UserControl
	{
		#region Fields

		private const int address1Column = 0;
		private const int address1EditColumn = 1;
		private const int address2Column = 2;
		private const int address2EditColumn = 3;
		private const int cityColumn = 4;
		private const int countyColumn = 5;
		private const int regionColumn = 6;
		private const int postCodeColumn = 7;
		private const int postCode2Column = 8;
		private const int countryColumn = 9;
		...
		private Business.com.ses.ws.AddressHygiene.OutputAddress OutAddress
		{
			get 
			{
		...
		public bool DoAddressHygiene(Business.com.ses.ws.AddressHygiene.Address address, bool enableSuggestionList)
		{
			ResetData();
		...
		private void DisplaySuggestionList()
		{
			...
			SuggestionListDataGrid.Columns[address1Column].Visible = true;
			SuggestionListDataGrid.Columns[address1EditColumn].Visible = false;
			SuggestionListDataGrid.Columns[address2Column].Visible = true;
			SuggestionListDataGrid.Columns[address2EditColumn].Visible = false;
			if (OutAddress.SuggestionListInformation.Status == SuggestionListStatus.AddressLineSuggestionListGenerated)
			{
				//Suggestion list contains a range of primary addresses. Have user manually input the address #
				SuggestionListDataGrid.Columns[address1Column].Visible = false;
				SuggestionListDataGrid.Columns[address1EditColumn].Visible = true;
			}
			else if (OutAddress.SuggestionListInformation.Status == SuggestionListStatus.SecondaryAddressLineSuggestionListGenerated)
			{
				//Suggestion list contains a range of secondary addresses. Have user manually input the address #
				SuggestionListDataGrid.Columns[address2Column].Visible = false;
				SuggestionListDataGrid.Columns[address2EditColumn].Visible = true;
			}
		...
		private DataView ConvertSuggestionListToDataView(Business.com.ses.ws.AddressHygiene.Address[] suggestionList)
		{
			DataView dv = new DataView(ConvertSuggestionListToDataTable(suggestionList));
			return dv;
		}

		private DataTable ConvertSuggestionListToDataTable(Business.com.ses.ws.AddressHygiene.Address[] suggestionList)
		{
		...
		private void SuggestionListDataGrid_ItemCommand(object source, DataGridCommandEventArgs e)
		{
			if(e.CommandName == "Select")
			{
				...
				AddressChanged = true;
				DoAddressHygiene(OutAddress, true);
			}
		}
		...
		private void SetInputAddressFromDataGridItem(DataGridItem e) 
		{
			if (OutAddress.SuggestionListInformation.Status == SuggestionListStatus.AddressLineSuggestionListGenerated)
			{
				OutAddress.Address1 = ((TextBox)e.Cells[address1EditColumn].FindControl("Address1TextBox")).Text.Replace("&nbsp;", "");
				OutAddress.Address2 = e.Cells[address2Column].Text.Replace("&nbsp;", "");
			}
			else if (OutAddress.SuggestionListInformation.Status == SuggestionListStatus.SecondaryAddressLineSuggestionListGenerated)
			{
				OutAddress.Address1 = e.Cells[address1Column].Text.Replace("&nbsp;", "");
				OutAddress.Address2 = ((TextBox)e.Cells[address2EditColumn].FindControl("Address2TextBox")).Text.Replace("&nbsp;", "");
			}
			else
			{
				OutAddress.Address1 = e.Cells[address1Column].Text.Replace("&nbsp;", "");
		...
		{
			AddressHygieneErrorCodes addressHygieneErrorCodes = new AddressHygieneErrorCodes();
