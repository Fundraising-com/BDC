...
{
	public partial class OrdersNotInvoiced : QSPFulfillment.CommonWeb.QSPPage
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!IsPostBack)
			{
				...
				OrdersNotInvoicedDG.SelectedIndex = 0;
				BindOrdersNotInvoicedGrid(strSearchField, strStartDate, strEndDate);
			}
		}


		#region Orders
		public void BindOrdersNotInvoicedGrid(string strSearchField, string strFromDate, string strToDate)
		{
			//Get all orders not invoiced by date range
			ds = aInvoiceDataAccess.GetAllOrdersNotInvoiced(strFromDate, strToDate);

			dv = ds.Tables[0].DefaultView;
			//Show the total number of records in the set
			if(strSearchField.ToString() == "")
			{
				int totalOrders = ds.Tables[0].Rows.Count;
				ShowOrdersStats(totalOrders);
			}
			else
			{
				lblOrdersNotInvoiced.Text = "";
			}
			if(strSearchField.Length > 0)
			{
				if(ddlStatus.SelectedItem.Value.Trim()=="Name")
				{
					dv.RowFilter = "GroupName LIKE '" + Regex.Replace(strSearchField,"'","''") + "%'";
					if (Session["OrdersSortField"] == null) Session["OrdersSortField"] = "GroupName";
				}
				else if(ddlStatus.SelectedItem.Value.Trim()=="AccountID")
				{
					dv.RowFilter = "AccountID = " + Convert.ToInt32(Regex.Replace(strSearchField,"'","''"));
					if (Session["OrdersSortField"] == null) Session["OrdersSortField"] = "AccountID";
				}
				else if(ddlStatus.SelectedItem.Value.Trim()=="OrderID")
				{
					dv.RowFilter = "OrderID = " + Convert.ToInt32(Regex.Replace(strSearchField,"'","''"));
					if (Session["OrdersSortField"] == null) Session["OrdersSortField"] = "OrderID";
				}
				else if(ddlStatus.SelectedItem.Value.Trim()=="CampaignID")
				{
					dv.RowFilter = "CampaignID = " + Convert.ToInt32(Regex.Replace(strSearchField,"'","''"));
					if (Session["OrdersSortField"] == null) Session["OrdersSortField"] = "CampaignID";
				}

			}
			dv.Sort = (string)Session["OrdersSortField"];
			// bind to the Data
			OrdersNotInvoicedDG.DataSource = dv;
			OrdersNotInvoicedDG.DataBind();
		}

		public void ShowOrdersStats(int nCount)
		{
			if (nCount == 0){lblOrdersNotInvoiced.Text = "No Records Found."; }
			else
			{
				int nStartOfSet = (nCount > 0) ? (OrdersNotInvoicedDG.CurrentPageIndex*OrdersNotInvoicedDG.PageSize+1) : 0;
				int nEndPage = (OrdersNotInvoicedDG.CurrentPageIndex+1)*(OrdersNotInvoicedDG.PageSize);
				int nEndOfSet = (nEndPage > nCount) ? nCount : nEndPage;
				lblOrdersNotInvoiced.Text = String.Format("Records: {0}-{1} of {2}", nStartOfSet, nEndOfSet ,nCount);
				OrdersNotInvoicedDG.PagerStyle.Visible = (nCount <= OrdersNotInvoicedDG.PageSize) ? false : true;
			}
		}

		public void OrdersNotInvoicedDG_Page(Object sender, DataGridPageChangedEventArgs e)
		{
			if( OrdersNotInvoicedDG.SelectedIndex != -1 )
			{
				//undo the selection
				OrdersNotInvoicedDG.SelectedIndex = -1;
			}
			//used by built-in pager.  CurrentPageIndex already set
			OrdersNotInvoicedDG.CurrentPageIndex = e.NewPageIndex;
		...
		public void OrdersNotInvoicedDG_Sort(Object sender, DataGridSortCommandEventArgs e)
		{
			Session["OrdersSortField"] = (string)e.SortExpression;
			strSearchField = (string)Search.Text.Trim();
			strStartDate    = (string)FromDate.Text.Trim();
			strEndDate      = (string)ToDate.Text.Trim();
			BindOrdersNotInvoicedGrid(strSearchField, strStartDate, strEndDate);
		...
		{
			OrdersNotInvoicedDG.CurrentPageIndex = 0;
			OrdersNotInvoicedDG.SelectedIndex = 0;
		...
		public void OrdersNotInvoicedDG_Select(Object sender, DataGridCommandEventArgs e)
		{
			if (e.CommandName == "Adjustment")
			{
				//Set the SelectedIndex property to select an item in the DataGrid
				OrdersNotInvoicedDG.SelectedIndex=e.Item.ItemIndex;
		...
		public void BindAdjustmentDetails()
		{
			...
			if( OrdersNotInvoicedDG.SelectedIndex != -1)
			{
				int nOrderID  = Convert.ToInt32(OrdersNotInvoicedDG.DataKeys[OrdersNotInvoicedDG.SelectedIndex]);
				ds = aInvoiceDataAccess.GetAllOrdersNotInvoicedAdjustmentDetails(nOrderID);
				...
				OrdersNotInvoicedAdjustmentDG.DataSource = dv;
				OrdersNotInvoicedAdjustmentDG.DataBind();
			}
		}
		...
		public void ShowAdjustmentStats(int nCount)
		{
			...
			{
				int nStartOfSet = (nCount > 0) ? (OrdersNotInvoicedAdjustmentDG.CurrentPageIndex*OrdersNotInvoicedAdjustmentDG.PageSize+1) : 0;
				int nEndPage = (OrdersNotInvoicedAdjustmentDG.CurrentPageIndex+1)*(OrdersNotInvoicedAdjustmentDG.PageSize);
				int nEndOfSet = (nEndPage > nCount) ? nCount : nEndPage;
				lblAdjustmentRecords.Text = String.Format("Records: {0}-{1} of {2}", nStartOfSet, nEndOfSet ,nCount);
				OrdersNotInvoicedAdjustmentDG.PagerStyle.Visible = (nCount <= OrdersNotInvoicedAdjustmentDG.PageSize) ? false : true;
			}
		}

		public void OrdersNotInvoicedAdjustmentDG_Page(Object sender, DataGridPageChangedEventArgs e)
		{
			//used by built-in pager.  CurrentPageIndex already set
			OrdersNotInvoicedAdjustmentDG.CurrentPageIndex = e.NewPageIndex;
		...
		public void OrdersNotInvoicedAdjustmentDG_Sort(Object sender, DataGridSortCommandEventArgs e)
		{
		...
		public void Adjustment_PageIndexChanged(Object sender, EventArgs e)
		{
			...
			{
				OrdersNotInvoicedAdjustmentDG.PageSize = Int32.Parse(ddlNumInvAdj.SelectedItem.Value.Trim());
			}
			else
			{
				OrdersNotInvoicedAdjustmentDG.PageSize = 1000;
			}
			OrdersNotInvoicedAdjustmentDG.CurrentPageIndex = 0;
		...
		public void ShowAdjustmentDetails()
		{
			...
			OrdersNotInvoicedAdjustmentDG.Visible	= true;
		}
		...
		public void HideAdjustmentDetails()
		{
			...
			OrdersNotInvoicedAdjustmentDG.Visible	= false;
		}

		public void OrdersNotInvoicedAdjustmentDG_ItemDataBound(Object sender, DataGridItemEventArgs e)
		{
		...
		public void BindPaymentDetails()
		{
			...
			if( OrdersNotInvoicedDG.SelectedIndex != -1)
			{
				int nID  = Convert.ToInt32(OrdersNotInvoicedDG.DataKeys[OrdersNotInvoicedDG.SelectedIndex]);
				ds = aInvoiceDataAccess.GetAllOrdersNotInvoicedPaymentDetails(nID);
				...
				OrdersNotInvoicedPaymentDG.DataSource = dv;
				OrdersNotInvoicedPaymentDG.DataBind();
			}
		}
		...
		public void ShowPaymentStats(int nCount)
		{
			...
			{
				int nStartOfSet = (nCount > 0) ? (OrdersNotInvoicedPaymentDG.CurrentPageIndex*OrdersNotInvoicedPaymentDG.PageSize+1) : 0;
				int nEndPage = (OrdersNotInvoicedPaymentDG.CurrentPageIndex+1)*(OrdersNotInvoicedPaymentDG.PageSize);
				int nEndOfSet = (nEndPage > nCount) ? nCount : nEndPage;
				lblPaymentRecords.Text = String.Format("Records: {0}-{1} of {2}", nStartOfSet, nEndOfSet ,nCount);
				OrdersNotInvoicedPaymentDG.PagerStyle.Visible = (nCount <= OrdersNotInvoicedPaymentDG.PageSize) ? false : true;
			}
		}

		public void OrdersNotInvoicedPaymentDG_Page(Object sender, DataGridPageChangedEventArgs e)
		{
			//used by built-in pager.  CurrentPageIndex already set
			OrdersNotInvoicedPaymentDG.CurrentPageIndex = e.NewPageIndex;
		...
		public void doAddPayment(Object sender, DataGridCommandEventArgs e)
		{
			...
			if ( e.CommandName == "Add" )
			{
				...
				if (Amount.Text.ToString().Trim().Length != 0 && bOk == true)
				{
					...
					string sUrl = "GLPaymentTransaction.aspx?InvoiceID=" + Convert.ToInt32(OrdersNotInvoicedDG.DataKeys[OrdersNotInvoicedDG.SelectedIndex]) + "&PaymentID="+nPaymentID +"&Amount=" + Convert.ToDecimal(Amount.Text.Trim());
		...
		public void OrdersNotInvoicedPaymentDG_Sort(Object sender, DataGridSortCommandEventArgs e)
		{
		...
		public void Payment_PageIndexChanged(Object sender, EventArgs e)
		{
			...
			{
				OrdersNotInvoicedPaymentDG.PageSize = Int32.Parse(ddlNumInvPay.SelectedItem.Value.Trim());
			}
			else
			{
				OrdersNotInvoicedPaymentDG.PageSize = 1000;
			}
			OrdersNotInvoicedPaymentDG.CurrentPageIndex = 0;
		...
		public void ShowPaymentDetails()
		{
			...
			OrdersNotInvoicedPaymentDG.Visible	= true;
		}
		...
		public void HidePaymentDetails()
		{
			...
			OrdersNotInvoicedPaymentDG.Visible	= false;
		}
		public void OrdersNotInvoicedPaymentDG_ItemDataBound(Object sender, DataGridItemEventArgs e)
		{
			if(e.Item.ItemType == ListItemType.Item)
			{
		...
		public void OrdersNotInvoicedDG_SelectedIndexChanged(object sender, System.EventArgs e)
		{
