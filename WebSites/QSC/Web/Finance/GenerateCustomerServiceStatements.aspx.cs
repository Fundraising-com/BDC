...
namespace QSPFulfillment.CustomerService
{
	...
	/// </summary>
	public class GenerateCustomerServiceStatements : CustomerService.CustomerServicePage
	{
		...
		{
			GenerateCustomerServiceProfitStatements();
		}

		private void GenerateCustomerServiceProfitStatements() 
		{
			byte[] results;
			FileStream fs;
			...
			{
                if (!File.Exists("D:\\Statements\\" + row["FMID"].ToString() + "\\" + row["FMID"].ToString() + "_" + row["Account_ID"].ToString().PadLeft(6, '0') + "_" + row["Campaign_ID"].ToString().PadLeft(7, '0') + "_3.pdf"))
                {
                    ...
                    fs = new FileStream("D:\\Statements\\" + row["FMID"].ToString() + "\\" + row["FMID"].ToString() + "_" + row["Account_ID"].ToString().PadLeft(6, '0') + "_" + row["Campaign_ID"].ToString().PadLeft(7, '0') + "_3.pdf", FileMode.Create);
		...
		private byte[] CallReportServicesDirect(string CampaignID)
		{
			...
			try 
			{
				asyncResult = oRS.BeginRender(
					"CustomerServiceOrdersStatement"
//					"/QSP CA SystemsCustomerServiceOrdersStatement"
					, "PDF"
					, null
					, null
					, inputParams
