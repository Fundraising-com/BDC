...
{
	/// <summary>
	/// Summary description for DataGridObject.
	/// </summary>
	public class DataGridObject: System.Web.UI.WebControls.DataGrid
	{
		...
		public DataGridObject()
		{
			...
			this.SortExpression = "";
			//this.ItemCreated += new DataGridItemEventHandler(DataGridObject_ItemCreated);
		}
		...
		public int SearchDataRow(string columnID, string searchingValue)
		{
			//This function will not work if the DataGrid is sort
			//int indexFound = -1;
			if(this.DataSource.GetType() == typeof(DataViewObject))
			{
				DataViewObject aTable = (DataViewObject)this.DataSource;
				for (int indexFound=0; indexFound<aTable.Table.Rows.Count;indexFound++)
				{
					if (aTable.Table.Rows[indexFound][columnID].ToString() == searchingValue)
						return indexFound;
				}
			}
			return -1;
		}
		public void ShowDataGridItem(int index){
			if ( index > 0)
			{
				int page = Convert.ToInt32(Math.Floor((double)(index/this.PageSize)));
				this.CurrentPageIndex = page;
				//this.SelectedIndex = index % this.PageSize;
				this.Items[index % this.PageSize].BackColor = this.SelectedItemStyle.BackColor;
				this.Items[index % this.PageSize].ForeColor = this.SelectedItemStyle.ForeColor;
				this.Items[index % this.PageSize].Font.CopyFrom(this.SelectedItemStyle.Font);
			}
		}
