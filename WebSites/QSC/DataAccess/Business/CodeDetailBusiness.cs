namespace QSPFulfillment.DataAccess.Business
{
	...
	using dataAccessRef =QSPFulfillment.DataAccess.Data.CodeDetailData;
	...
	///     collection Day.
	/// </summary>
	public class CodeDetailBusiness : QSPFulfillment.DataAccess.Business.BusinessSystem
	{
		dataAccessRef dataAccess = new dataAccessRef();
		public CodeDetailBusiness(Message MessageManager):base(MessageManager)
		{
		
		}
		public CodeDetailBusiness(bool AsMessageManager):base(AsMessageManager)
		{
		...
		public void SelectByCodeHeaderInstance(DataTable Table,int CodeHeaderInstance)
		{
			try
			{
				dataAccess.SelectByCodeHeaderInstance(Table,CodeHeaderInstance);
			}
			catch (Exception ex)
			{	
				ManageError(ex);
		...
		{
			bool IsValid = true;
			IsValid &= IsValid_RequiredField(Row,tableRef.FLD_CODEHEADERINSTANCE,"CodeHeaderInstance");
		...
		private bool IsValids_FieldLength(DataRow Row)
		{
			...
			isValid &= IsValid_FieldLength(Row, tableRef.FLD_ADPCODE,"", tableRef.FLD_ADPCODE_LENGTH);
			return isValid;
		}
