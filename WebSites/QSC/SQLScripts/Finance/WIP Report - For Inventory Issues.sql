SELECT		dm.Firstname + ' ' + dm.Lastname DM,
			...
			o.CampaignID,
			o.OrderID,
			o.Date OrderLoadedInFFSDate,
			o.ProductCode,
			p.OracleCode SAPMaterialCode,
			p.Product_Name ProductName,
			CASE WHEN o.OrderQualifierID IN (39001,39002) THEN 'Landed' WHEN o.OrderQualifierID IN (39009) THEN 'Online' ELSE 'N/A' END OrderSource,
			CASE cod.ProductType WHEN 46001 THEN 1 ELSE cod.Quantity END Quantity,
			(o.TotalPrice - o.PostageAmount -
			...
			(1 - CASE WHEN o.SectionType = 1 THEN CASE WHEN o.CAccountCodeClass = 'FM' THEN 0.00
					  ...
					  WHEN o.SectionType = 15 THEN CASE WHEN o.OrderQualifierID = 39022 THEN 0.00 
													...
													END
					  ELSE 0
			END) - CASE WHEN o.SectionType IN (1) THEN (o.tax+o.Tax2) ELSE 0.00 END NetSalesInProgress,
			TotalPrice + CASE WHEN o.SectionType IN (1, 3, 7, 10, 11) THEN (o.tax+o.Tax2) ELSE 0.00 END GrossSalesInProgress,
...
BEGIN
	...
	WHERE CONVERT(Datetime,Convert(Varchar(10),#WIP.OrderLoadedInFFSDate,101)) >= @EffectiveFrom
...
SELECT		ProductLine,
			...
			ProductCode,
			ProductName,
			SUM(Quantity) QuantityStillToBeShipped
FROM		#WIP
where		ProductLine not in ('Magazine','To Remember This')
