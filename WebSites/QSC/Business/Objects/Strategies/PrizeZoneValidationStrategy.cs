...
namespace Business.Objects.Strategies
{
	...
	/// </summary>
	internal class PrizeZoneValidationStrategy : CampaignProgramValidationStrategy
	{
		private const string PROGRAM_NAME = "Cumulative (Prize Zone)";

        internal PrizeZoneValidationStrategy(Message messageManager) : base(messageManager) { }
...
//			}
			...
			{
				CurrentMessageManager.Add(CurrentMessageManager.FormatErrorMessage(Message.ERRMSG_PROGRAM_CANNOT_BE_RUN_WITH_2, new string[] {PROGRAM_NAME, "Cumulative (Go For The Gold)"}));
				isValid = false;
			}
