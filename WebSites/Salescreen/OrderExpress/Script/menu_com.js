...
function OpenMenu(){	
	...
	if(CCnt!=null){
		...
		status=this.LinkTxt;
		//new code
		//alert(parseInt(ChCntTL.top));
		if(CCnt!=null){
			HideDropDowns(SubTp, SubLt, CCW, CCH);
		}
		//HideDropDowns(ChCntTL);
}
...
function HideDropDowns(ptop, pleft, pwidth, pheight){
	try {		
		var j = 0;
		if ( document.forms.length > 0 ){
			...
			for (i = 0; i < LEN; i++){												
					if(document.forms[0].elements[i].type == 'select-one'){
						...
						MenuStartTop = ptop;
						var MenuEndTop;
						MenuEndTop = ptop + pheight;
						var MenuStartLeft;
						MenuStartLeft = pleft;
						var MenuEndLeft;
						MenuEndLeft = pleft + pwidth;
						var objStartTop;
						objStartTop = findPosY(document.forms[0].elements[i]);
						var objEndTop;
						objEndTop = objStartTop + document.forms[0].elements[i].offsetHeight;
						var objStartLeft;
						objStartLeft = findPosX(document.forms[0].elements[i]);
						var objEndLeft; 
						objEndLeft = objStartLeft + document.forms[0].elements[i].offsetWidth;
						/*alert('objStartTop =' + objStartTop + 
							  ', objEndTop =' + objEndTop + 
							  ', objStartLeft =' + objStartLeft + 
							  ', objEndLeft =' + objEndLeft +
							  ', MenuStartTop =' + MenuStartTop +
							  ', MenuEndTop =' + MenuEndTop +
							  ', MenuStartLeft =' + MenuStartLeft +
							  ', MenuEndLeft =' + MenuEndLeft )*/
						if (((objStartTop <= MenuEndTop) &&
							(objEndTop >= MenuStartTop)) &&
							((objStartLeft <= MenuEndLeft) &&
							(objEndLeft >= MenuStartLeft))) {						
							document.forms[0].elements[i].style.visibility = M_Hide;
						} else {
							document.forms[0].elements[i].style.visibility = M_Show;							
						}
...
function ShowDropDowns(){
	...
	if ( document.forms.length > 0 ){
		var LEN = document.forms[0].elements.length;
		for (i = 0; i < LEN; i++){
			//if(document.forms[0].elements[i].type == 'select-one' && j < 35){
			//	j++;
			//if(document.forms[0].elements[i].type == 'select-one' && j < 35){
			//	j++;				
			if (document.forms[0].elements[i].style.visibility == M_Hide)
				document.forms[0].elements[i].style.visibility=M_Show;
			//}
		}
	}
...
function showDiv(oDiv){
	...
	alert(oHelperIframe.type + " " + oDiv.id);
	oHelperIframe.width = oDiv.width - 1;
	oHelperIframe.height = oDiv.style.height - 1;
