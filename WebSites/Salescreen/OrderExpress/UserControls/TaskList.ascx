...
<%@ Control Language="c#" Inherits="QSP.OrderExpress.Web.UserControls.TaskList" Codebehind="TaskList.ascx.cs" %>
...
<TABLE id="Table5" cellSpacing="0" cellPadding="0" align="left" border="0">
	...
	<TR>
		<TD><!--DataGrid  -->
			<cc2:sorteddatagrid id="dtgTaskItems" runat="server" ShowFooter="True" AutoGenerateColumns="False" 
			DataSource="<%# DVTask %>" BorderColor="#CCCCCC" CssClass=GridStyle
			BackColor="White" CellPadding="2" AllowSorting="True" AllowPaging="True" PageSize="30" width=700px>
				...
				<Columns>
					<asp:TemplateColumn>
						...
						<ItemTemplate>
							<asp:ImageButton id="imgBtnDetail" height=15px runat="server" CausesValidation="False" ImageUrl="~/images/BtnDetail.gif" CommandArgument='<%# DataBinder.Eval(Container, "DataItem.task_id") %>' />
						</ItemTemplate>
					</asp:TemplateColumn>
					<asp:TemplateColumn SortExpression="task_type_name" HeaderText="Type">
						<ItemStyle Wrap="False" ></ItemStyle>
						<ItemTemplate>
							<asp:Label id="lblTaskTypeName" runat="server" Width="150px" Text='<%# DataBinder.Eval(Container, "DataItem.task_type_name") %>' />
						</ItemTemplate>
					</asp:TemplateColumn>
					<asp:TemplateColumn SortExpression="task_name" HeaderText="Task Name">
						<ItemStyle Wrap="False" Width="100%"></ItemStyle>
						<ItemTemplate>
							<asp:Label id="lblTaskName" runat="server" Text='<%# DataBinder.Eval(Container, "DataItem.task_name") %>' />
						</ItemTemplate>
					</asp:TemplateColumn>
				</Columns>
			</cc2:sorteddatagrid>
