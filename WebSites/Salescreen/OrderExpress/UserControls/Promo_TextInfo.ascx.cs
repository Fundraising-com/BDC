...
using dataRef = QSPForm.Common.DataDef.Promo_TextTable;
using QSP.OrderExpress.Web.Code;

namespace QSP.OrderExpress.Web.UserControls {
    ///<summary>User Information - read only</summary>
    public partial class Promo_TextInfo : BaseWebFormControl {
        #region Item Declarations
        private CommonUtility util = new CommonUtility();
        protected dataRef dtblPromo_Text;
        ...
        public dataRef DataSource {
            get {
                return dtblPromo_Text;
            }
            set {
                dtblPromo_Text = value;
            }
        }
        ...
        public override void DataBind() {
            ...
            if (this.DataSource.Rows.Count > 0) {
                ...
                this.lblName.Text = row[dataRef.FLD_PROMO_TEXT_NAME].ToString();
                ...
                this.lblText.Text = row[dataRef.FLD_TEXT].ToString();
        ...
        private void FillList(int Promo_TextID) {
            QSPForm.Business.Promo_TextSubdivisionSystem regSys = new QSPForm.Business.Promo_TextSubdivisionSystem();
            DataTable table = regSys.SelectAllByPromo_TextID(Promo_TextID);
            foreach (DataRow row in table.Rows) {
                lbxCurrentSubdivision.Items.Add(new ListItem(row[QSPForm.Common.DataDef.Promo_TextSubdivisionTable.FLD_SUBDIVISION_CODE] + " - " + row[QSPForm.Common.DataDef.Promo_TextSubdivisionTable.FLD_SUBDIVISION_NAME_1], ""));
            }
        }
    }
}