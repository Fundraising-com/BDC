...
using dataRef = QSPForm.Common.DataDef.LogoTable;
using QSP.OrderExpress.Web.Code;

namespace QSP.OrderExpress.Web.UserControls {
    ///<summary>User Information - read only</summary>
    public partial class LogoInfo : BaseWebFormControl {
        #region Item Declarations
        private CommonUtility util = new CommonUtility();
        protected dataRef dtblLogo;
        ...
        private void InitializeUI() {
            trFMID.Visible = (Page.Role != QSPForm.Business.AuthSystem.ROLE_FM);
        }
        ...
        public override void DataBind() {
            ...
            if (this.DataSource.Rows.Count > 0) {
                ...
                this.lblName.Text = row[dataRef.FLD_LOGO_NAME].ToString();
                ...
                this.imgBtnDetail.ImageUrl = QSPForm.Common.QSPFormConfiguration.LogoImagePreviewPath +
        ...
        private void FillList(int LogoID) {

            QSPForm.Business.LogoSubdivisionSystem regSys = new QSPForm.Business.LogoSubdivisionSystem();
            DataTable table = regSys.SelectAllByLogoID(LogoID);
            foreach (DataRow row in table.Rows) {
                lbxCurrentSubdivision.Items.Add(new ListItem(row[QSPForm.Common.DataDef.LogoSubdivisionTable.FLD_SUBDIVISION_CODE] + " - " + row[QSPForm.Common.DataDef.LogoSubdivisionTable.FLD_SUBDIVISION_NAME_1], ""));
            }
        }
    }
}