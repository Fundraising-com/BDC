...
<%@ Control Language="c#" Inherits="QSP.OrderExpress.Web.UserControls.AccountList_AddOrder" Codebehind="AccountList_AddOrder.ascx.cs" %>
	...
	<TR>
		<TD width="100%"><!--DataGrid  --><cc2:sorteddatagrid id=dtgAccount runat="server" SearchMode="0" ShowFooter="True" DataSource="<%# DVAccount %>" AutoGenerateColumns="False" BorderColor="#CCCCCC" CssClass=GridStyle CellPadding="3" AllowSorting="True" AllowPaging="True" Width="100%" PageSize="30" onselectedindexchanged="dtgAccount_SelectedIndexChanged">
				...
				<Columns>
					<asp:TemplateColumn>
						<ItemTemplate>
							<asp:imagebutton id="imgBtnSelect" runat="server" ImageUrl="~/images/BtnAddOrder.gif" CommandName="Select"
					...
					<asp:TemplateColumn visible=false>
						<ItemTemplate>
							<asp:Label id="lblStatusID" runat="server" Text='<%# DataBinder.Eval(Container, "DataItem.account_status_id")%>'>								
							</asp:Label>
						</ItemTemplate>
					</asp:TemplateColumn>
					<asp:BoundColumn DataField="account_status_short_description" SortExpression="account_status_short_description"
					...
					<asp:TemplateColumn SortExpression="fiscal_year" HeaderText="FY" visible="false">
						<ItemTemplate>
							<asp:Label id="lblFiscalYear" runat="server" Text='<%# DataBinder.Eval(Container, "DataItem.fiscal_year") %>'>
							</asp:Label>
						</ItemTemplate>
					</asp:TemplateColumn>
