...
namespace QSP.OrderExpress.Web.UserControls {
    ...
    public partial class Logo : BaseWebFormControl {
        ...
        private void Upload() {
            try {
                if (ValidateFile(ctrlUpload.PostedFile)) {
                    ...
                    this.TempFileName = String.Format("{0:dd-MM-yyyy-HHmmss}", DateTime.Now) + "-" + this.Session.SessionID + "1." + FileExtension;
                    this.TempPreviewFileName = String.Format("{0:dd-MM-yyyy-HHmmss}", DateTime.Now) + "-" + this.Session.SessionID + "2." + DefaultFileExtension;
        ...
        public void CreateImageAndPreview() {
            if (System.IO.File.Exists(this.TempPath + this.TempFileName)) {
                ...
                */
                ...
                File.Copy(this.TempPath + this.TempFileName, QSPForm.Common.QSPFormConfiguration.LogoImagePath +
                                    this.DataSource.Logo.Rows[0][QSPForm.Common.DataDef.LogoTable.FLD_PKID] +
                                    "." + this.FileExtension);
        ...
        public void AdjustMode() {
            string cssClass = "";
            bool readOnly = false;
            if (this.Mode == PageMode.ReadOnly) {
                cssClass = "ReadOnlyTextBox";
                readOnly = true;
            }

            //set all applicable control to this skinID
            txtDescription.CssClass = cssClass;
            txtFMID.CssClass = cssClass;
            txtFMName.CssClass = cssClass;
            txtName.CssClass = cssClass;

            txtDescription.ReadOnly = readOnly;
            txtFMID.ReadOnly = readOnly;
            txtFMName.ReadOnly = readOnly;
            txtName.ReadOnly = readOnly;

            chkEnabled.Enabled = !readOnly;
            chkNational.Enabled = !readOnly;
            ddlImageCategory.Enabled = !readOnly;
        }
    }
}