...
namespace QSP.OrderExpress.Web.UserControls {
    ...
    /// </summary>
    public partial class OrderStep_Selection : BaseWebFormControl {
        ...
        private void imgBtnBack_Click(object sender, System.Web.UI.ImageClickEventArgs e) {
            ...
            string url = "OrderStep_Search.aspx?";
            Response.Redirect(url + "&AccID=" + AccountID.ToString());
        }
        protected void dtgForm_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            if (dtgForm.SelectedIndex > -1)
            {
                int formId = GetFormID(dtgForm.Items[dtgForm.SelectedIndex]);
                ...
                if (isPaInSystem)
                {
                    string ID = dtgForm.DataKeys[dtgForm.SelectedIndex].ToString();
                    string url = this.Page.GetPageToGo(QSPForm.Business.AppItem.OrderForm_Step3);
                    Response.Redirect(url + "&FormID=" + ID + "&CampID=" + CampaignID.ToString());
                }
                else
                {
                    string script = "<script type=\"text/javascript\">\n";
                    script += "  alert(\"A PA is required to place an order.\");\n";
                    script += "</script>\n";

                    Page.RegisterStartupScript("EnterPA", script);

                    dtgForm.SelectedIndex = -1;
                }
            }
        }
        ...
        private new void BindForm()
        {
            ...
            List<LinqEntity.Form> formList = formSystem.SelectByEntityTypeAndUserPermissions(EntityTypeEnum.Order, this.UserId, this.AccountID);
        ...
        private int GetFormID(DataGridItem dataGridItem) {
            return Convert.ToInt32(((Label)dataGridItem.FindControl("FormIDLabel")).Text);
        }
        public int UserId {
