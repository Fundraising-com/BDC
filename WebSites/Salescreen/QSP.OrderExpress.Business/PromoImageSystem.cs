...
namespace QSPForm.Business
{
	...
	using dataAccessRef = QSPForm.Data.Promo_Image;
	...
	public class PromoImageSystem : BusinessSystem
	{
		
		dataAccessRef objDataAccess;
		
		public PromoImageSystem()
		{
...
//			ruleTable.Rows.Add(ruleRow);
        ...
        */
        ...
        public dataDef SelectOne(int PromoID, int FormatID)
        {
            dataDef dTbl;
            dTbl = objDataAccess.SelectOne(PromoID,FormatID);
            return dTbl;
        }
        public dataDef SelectAllByOrderID(int OrderID, int FormatID)
        {
            dataDef dTbl;
            dTbl = objDataAccess.SelectAllByOrderID(OrderID, FormatID);
            return dTbl;
        }

        //public string GetImage(int PromoID, int Resolution)
		public byte[] GetImage(int PromoID, int Resolution, int FormatID)
		{	
			try
			{
				string path = ""; 
				...
				DataTable dtbl = SelectOne(PromoID,FormatID);
                string imgID;
				if (dtbl.Rows.Count > 0)
				{
                    imgID = dtbl.Rows[0][dataDef.FLD_PKID].ToString();
					if (Resolution == 1)
						path += imgID + "." + previewFileExt;
					else
						path += imgID + "." + dtbl.Rows[0][dataDef.FLD_FILE_EXTENSION].ToString();
        ...
        public AV_AssetTable SelectAllPromoURLByOrderID(int OrderID, int Format)
        {
            dataDef dTbl;
            dTbl = SelectAllByOrderID(OrderID, Format);
            AV_AssetTable tbl = new AV_AssetTable();
            string path = QSPForm.Common.QSPFormConfiguration.PromoImagePreviewPath;
            string previewFileExt = QSPForm.Common.QSPFormConfiguration.ImagePreviewFileExtension;
            foreach (DataRow r in dTbl.Rows)
            {
                ...
                newRow[AV_AssetTable.FLD_ASSET_URL] = sImageUrl;
                newRow[AV_AssetTable.FLD_ASSET_HR_FILE_EXTENSION] = r[dataDef.FLD_FILE_EXTENSION].ToString();
                newRow[QSPForm.Common.DataDef.AV_AssetTable.FLD_ASSET_NAME] = r[dataDef.FLD_NAME].ToString();
                newRow[QSPForm.Common.DataDef.AV_AssetTable.FLD_ASSET_DESCRIPTION] = r[dataDef.FLD_DESCRIPTION].ToString();
                newRow[QSPForm.Common.DataDef.AV_AssetTable.FLD_ASSET_CATEGORY] = r[dataDef.FLD_CATEGORY].ToString();
                tbl.Rows.Add(newRow);
            }
