...
namespace QSPForm.Business
{
	...
	using dataAccessRef = QSPForm.Data.OrderDetailTax;
	...
	public class OrderDetailTaxSystem : BusinessSystem
	{
		
		dataAccessRef objDataAccess;
		
		public OrderDetailTaxSystem()
		{
		...
		private bool IsValid_RequiredFields(DataRow row)
		{
			bool IsValid = true;
			//Order Detail
			IsValid &= IsValid_RequiredField(row, dataDef.FLD_ORDER_DETAIL_ID, "Order");
			//Rate
			IsValid &= IsValid_RequiredField(row, dataDef.FLD_TAX_RATE, "Tax rate");
			//Amount
			IsValid &= IsValid_RequiredField(row, dataDef.FLD_AMOUNT, "Amount");
		...
		public dataDef SelectAllByOrderDetailID(int OrderDetailID)
		{			
			dataDef dTbl;
			//
			// Get the user DataTable from the DataLayer
			//
			dTbl = objDataAccess.SelectAllWorder_detail_idLogic(OrderDetailID);				
			
			return dTbl;			
		}
		
		public void CalculateByOrderID(int OrderID)
		{			
			//
			// Get the user DataTable from the DataLayer
			//
			objDataAccess.CalculateWorder_idLogic(OrderID);				
