...
namespace EFundraisingCRMWeb.Components.User.PaymentInformation
{
	...
	public partial class PaymentOptions : System.Web.UI.UserControl
	{
		...
		protected void paymentMethodDropDownList_SelectedIndexChanged(object sender, System.EventArgs e)
		{
            ...
            if (paymentMethodID == 2 || paymentMethodID == 3 || paymentMethodID == 8 || paymentMethodID == 9 || paymentMethodID == 15 )
            {
                PaymentTermDropdownList.SelectedIndex = 4;
                ProcessButton.Visible = true;
            }
            else if (paymentMethodID == 1 || paymentMethodID == 11 ||  paymentMethodID == 13 || paymentMethodID == 14)
            {
                PaymentTermDropdownList.SelectedIndex = 0;
            }
            else if (paymentMethodID == 16)
            {
                PaymentTermDropdownList.SelectedIndex = 6;
            }
		...
		private void ProcessCC()
		{
			try
			{
				...
				*/
				...
				{
                  Logger.Debug("Sales Screen: Process CC " + msg);
				}
			}
			catch(Exception x)
			{
				Logger.Error("Sales Screen: Process CC ",x);
