...
{
	/// <summary>
	/// Purpose: Data Access class for the table 'PhoneNumberOrganization'.
	/// </summary>
	public class Phone_number_entity : DBTableOperation
	{


		//Stored procedure parameter names
		public const string PARAM_PKID = "@iphone_number_entity_id";
		...
		public const string PARAM_TYPE ="@iphone_number_type_id";

		public const string PARAM_PHONE_NUMBER ="@sphone_number";
		
		//
		// Stored procedure names for each operation
		private const string SQL_PROC_DELETE = "pr_phone_number_entity_Delete";
		private const string SQL_PROC_INSERT = "pr_phone_number_entity_Insert";
		private const string SQL_PROC_UPDATE = "pr_phone_number_entity_Update";	
		private const string SQL_PROC_SELECT_ONE = "pr_phone_number_entity_SelectOne";
        private const string SQL_PROC_SELECT_ONE_LOGIC = "pr_phone_number_SelectOne";
		private const string SQL_PROC_SELECT_ALL = "pr_phone_number_entity_SelectAll";
		...
		protected override SqlCommand GetInsertCommand()
		{
			if ( insertCommand == null )
			{
				...
				sqlParams.Add(new SqlParameter(PARAM_PHONE_NUMBER, SqlDbType.VarChar, 20));			
				...
				sqlParams[PARAM_PHONE_NUMBER_ID].Direction = ParameterDirection.InputOutput;	
				//
				...
				sqlParams[PARAM_PHONE_NUMBER].SourceColumn = dataDef.FLD_PHONE_NUMBER;			
		...
		{
			get{return dataDef.TBL_PHONE_NUMBER_ENTITY;}
		}
        ...
        public dataDef SelectOne(int AddressEntityID)
        {
            ...
            cmdToExecute.CommandType = CommandType.StoredProcedure;
            cmdToExecute.Parameters.Add(new SqlParameter(PARAM_PHONE_NUMBER_ID, SqlDbType.Int, 4, ParameterDirection.Input, false, 10, 0, "", DataRowVersion.Proposed, AddressEntityID));
            Select(cmdToExecute, Table);
		...
		public dataDef SelectWentity_idLogic(int EntityTypeID, int EntityID)
		{
			...
			SqlCommand	cmdToExecute = new SqlCommand();
			cmdToExecute.CommandText = "dbo.pr_phone_number_entity_SelectAllWentity_idLogic";
			cmdToExecute.CommandType = CommandType.StoredProcedure;
						...
						if (!reassigned)
						{
							...
							if (DV.Count > 0)
							{
								...
								if (!row.IsNull(PhoneNumberEntityTable.FLD_PHONE_NUMBER))
									phone_number = row[PhoneNumberEntityTable.FLD_PHONE_NUMBER].ToString();
								int PhoneNumberID = Convert.ToInt32(row[PhoneNumberEntityTable.FLD_PHONE_NUMBER_ID]);
						

								DV.RowFilter = "ISNULL(" + PhoneNumberEntityTable.FLD_PHONE_NUMBER  + ", '') = '" + phone_number + "'"; 
								...
								if (DV.Count > 0)
								{
									...
									foreach(DataRowView dvRow in DV)
									{
										...
										if (!duplRow.IsNull(PhoneNumberEntityTable.FLD_PHONE_NUMBER_ID))
										{
											if (Convert.ToInt32(duplRow[PhoneNumberEntityTable.FLD_PHONE_NUMBER_ID]) == PhoneNumberID)
												dvRow[PhoneNumberEntityTable.FLD_PHONE_NUMBER_ID] = PhoneNumberID;
										}
										else
											dvRow[PhoneNumberEntityTable.FLD_PHONE_NUMBER_ID] = PhoneNumberID;
