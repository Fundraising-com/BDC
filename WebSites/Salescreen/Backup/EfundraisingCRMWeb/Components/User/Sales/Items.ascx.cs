	...
	public partial class Items : System.Web.UI.UserControl
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            if (!IsPostBack)
            {
                ...
                string conn = "";  
              efundraising.EFundraisingCRM.Linq.QSPFulfillmentDataContext db = new efundraising.EFundraisingCRM.Linq.QSPFulfillmentDataContext();
              //if (EFundraisingCRMWeb.Components.Server.ManageSaleScreen.IsProd())
              //{
                 // conn = ConfigurationManager.ConnectionStrings["QSPFulfillmentConnectionString"].ConnectionString;
                 // db = new efundraising.EFundraisingCRM.Linq.QSPFulfillmentDataContext(conn);
              //}
              //else
              //{
              conn = ConfigurationManager.ConnectionStrings["QSPFulfillmentConnectionStringDEV"].ConnectionString;
              db = new efundraising.EFundraisingCRM.Linq.QSPFulfillmentDataContext(conn);
              //}
		...
		public DataTable GetDataFromDatagrid(int itemIndexToDelete, ref decimal totalProfit)
		{
			...
			{
				efundraising.Diagnostics.Logger.LogError("Error in GetDataFromDatagrid", ex);
			}
                               ...
                               /*for (int i = 0; i < dt.Rows.Count; i++)
                                ...
                                }else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            margin = d.ToString("C").Remove(0, 1);
                                        }
                                        
                                    
                                        try
                                        {
                                            ddList.SelectedValue = margin;
                                        }
                                        catch (Exception x)
                                        {
		...
		public void GetTotals(int saleID)
		{
			...
			try
			{
                ...
                decimal gstOnAdj = 0;
                decimal pstOnAdj = 0;
                Adjustment[] adj = Adjustment.GetAdjustmentsBySaleID(saleID);
                foreach (Adjustment a in adj)
                {
                    if (a.ReasonID == 23)
                    {
                        adjustment = Convert.ToDecimal(a.AdjustmentOnSaleAmount);
                    }
                }
                if (!EFundraisingCRMWeb.Components.Server.ManageSaleScreen.IsSaleTaxExempt(saleID))
                {
                    if (adjustment != 0)
                    {
                        EFundraisingCRMWeb.Components.Server.ManageSaleScreen.CalculateTaxes(adjustment, s.ClientId, s.ClientSequenceCode, ref pstOnAdj, ref gstOnAdj);
                    }
                }
                ...
                InvoiceAmountTextBox.Text = (s.TotalAmount - Convert.ToDouble(adjustment) - Convert.ToDouble(gstOnAdj) - Convert.ToDouble(pstOnAdj)).ToString("C");
				ShippingFeeTextBox.Text = s.ShippingFees.ToString("C");
                //DiscountTextBox.Text = s.ShippingFees.ToString("C");
				...
				TotalTextBox.Text = (Convert.ToDecimal(s.TotalAmount) - pst -pstOnAdj - gst -gstOnAdj - shipping).ToString("C");
			}
			catch (Exception ex)
			{
				efundraising.Diagnostics.Logger.LogError("Sales Screen: Items - GetTotals", ex);
			}
		...
		public bool CalculateTotals(bool validate, ref int nbProductClass, ref DataTable productList)
		{
            ...
            try
			{
				...
				if (dgDataTable.Rows.Count > 0)
				{
					...
					foreach (DataRow dr in dgDataTable.Rows)
					{
						...
						if (profitPercentage > 0)
						{
							...
							itemPriceTextBox.Text = (EFundraisingCRMWeb.Components.Server.ManageSaleScreen.GetItemPrice(sc,profitPercentage/100)).ToString("C");
						...
						{
                            line = 5;
							itemPriceTextBox.Text = EFundraisingCRMWeb.Components.Server.ManageSaleScreen.GetItemPrice(sc,0).ToString("C");
							dTotal = Convert.ToDecimal(itemPriceTextBox.Text.Substring(1))*qty;
							totalAmountTextBox.Text = dTotal.ToString("C");
                ...
                /*    int clientID = Convert.ToInt32(System.Web.HttpContext.Current.Session[Global.SessionVariables.CLIENT_ID]);
                    string clientSeqCode = System.Web.HttpContext.Current.Session[Global.SessionVariables.CLIENT_SEQUENCE_CODE].ToString();
                    efundraising.EFundraisingCRM.ClientAddress ca = efundraising.EFundraisingCRM.ClientAddress.GetClientAddressByIdSequenceAddressType(clientID, clientSeqCode, "ST");
*/
                    ...
                    EFundraisingCRMWeb.Components.Server.ManageSaleScreen.CalculateTaxesByProductClass(classTotals, clientId, clientSeq, ref totalGST, ref totalPST,adjustment,ref pstOnAdj, ref gstOnAdj );
                    ...
                    Surcharge = Surcharge;
                    InvoiceAmount = totalShipping + totalAmount + PST + GST + Surcharge - Discount; 
			...
			{
			     efundraising.Diagnostics.Logger.LogError("Error in CalculateTotals. line: " + line.ToString(), ex);
			}
		...
		public void DisableEverything()
		{
			...
			{
				efundraising.Diagnostics.Logger.LogError("Error in DisableEverything", ex);
			}
                    ...
                    foreach (DataRow dr in dgDataTable.Rows)
                    {
                        ...
                        if (baseLevel == "ProductClass")
                        {
                            ...
                            if (pcID == productClassID)
                            {
                                ...
                                ddprofit.SelectedValue = profit.ToMoney(false);
