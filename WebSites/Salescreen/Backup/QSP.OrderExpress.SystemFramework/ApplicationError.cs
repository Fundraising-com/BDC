...
{
	///<summary>ApplicationError</summary>
	public class ApplicationError
	{
		...
		public static void ManageError(Exception ex)
		{
			...
			if(ApplicationConfiguration.DBInsertEnabled)
				ErrorID = Log_Exception(ex, string.Empty);

			//send mail error with the link
			if(ErrorID != 0 && ApplicationConfiguration.SendMailErrorEnabled)
			{
				new SendMail(MailTypes.ErrorWebApplication,"http://www.qsp.com/admin/errors_main.asp?site_id=7&id=" + ErrorID + "\r\n",ApplicationConfiguration.MailFrom,ApplicationConfiguration.MailTo,ApplicationConfiguration.MailSmtp, ex);
			}
				//send mail without link
			else if(ApplicationConfiguration.SendMailErrorEnabled)
			{
				new SendMail(MailTypes.ErrorWebApplication,"",ApplicationConfiguration.MailFrom,ApplicationConfiguration.MailTo,ApplicationConfiguration.MailSmtp, ex);
			}


		}

        public static void LogErrorAndSuppressMail(Exception ex)
        {
            Log_Exception(ex, "Generated by security scan");
        }
        ...
        private static int Log_Exception(System.Exception ex, string sAdditionalComments)
		{
            ...
            string sViewstate               = HttpContext.Current.Server.UrlEncode(HttpContext.Current.Request.Form["__VIEWSTATE"]);

            if (sViewstate != null)
                sFormValues = sFormValues.Replace(sViewstate, string.Empty);
			...
			try
			{
				conn = new SqlConnection(QSPForm.SystemFramework.ApplicationConfiguration.ErrorConnectionString);
			}
			catch
			{
			...
			SqlCommand cmdAdd;
			cmdAdd = new SqlCommand( "QSPError.dbo.pr_Insert_Error_ASPNET", conn );
			...
			cmdAdd.Parameters.Add( "@SiteId",						7							);
