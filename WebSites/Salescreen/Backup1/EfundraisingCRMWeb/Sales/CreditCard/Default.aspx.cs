        ...
        protected void newProcessButton_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                if (nbClick == 1)
                {
                    ...
                    {
                        Logger.LogInfo("ProcessNew error " + ex);
                ...
                if (salesInfo.Count > 0)
                {
                    ...
                    if (amountToProcess <= selectedSaleTotal + 0.01 || isSaleSupport)
                    {
                        ...
                        if (!badZip)
                        {
                            ...
                            string responseID = oResponse.responseCode;
                           
                            /*authArray[3] = "0"; //oResponse.paymentId 
                            authArray[4] = oResponse.ErrorMessage;*/

                            Logger.LogInfo("CC Request for sale: " + salesInfo[0].ToString() + " bppsID:" + orderID + " **** " + "CC response code for sale: " + responseID + " **** " + "CC amount charged for sale: " + amount);
                            ...
                            if (oResponse.responseCode == "100")
                            {
                                ...
                                foreach (string[] sale in salesInfo)
                                {
                                    try
                                    {
                                        ...
                                        Comments currentComment = new Comments(int.MinValue,
                                            ...
                                            9, // other 
                                            DateTime.Now,
                                            "CC processed by PaymentTech OrderID: " + orderID + " for $" + amountToProcess + ". $" + processedAmountForSale + " of this amount was applied to this sale.");


                                        try
                                        {
                                            payments.Add(currentPayment);
                                            Logger.LogInfo("currentPayment");
                                            sales.Add(currentSale);
                                            Logger.LogInfo("currentSale");
                                            comments.Add(currentComment);
                                            Logger.LogInfo("currentComment");
                                        }
                                ...
                                //            // display success message to the user
                                            ...
                                            LabelStatus.Text = "Transaction Successful!<br>"
                                                ...
                                                + "PaymentTech OrderID: " + orderID + "<br>"
                                                + "Affected Sale(s):<br>";
                                            foreach (string[] sale in salesInfo)
                                            {
                                                LabelStatus.Text += sale[0] + "<br>";
