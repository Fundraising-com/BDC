	...
	public partial class Items : System.Web.UI.UserControl
	{
		...
		public void GetTotals(int saleID)
		{
			...
			try
			{
                ...
                TotalTextBox.Text = (Convert.ToDecimal(s.TotalAmount) + adjustment - pst - gst - shipping).ToString("C");
				...
				if (dgDataTable.Rows.Count > 0)
				{
					...
					foreach (DataRow dr in dgDataTable.Rows)
					{
						...
						if (profitPercentage > 0)
						{
                            ...
                            // hack for beef jerky product quantity more than 10 items itemprice should be $90
                            //if ((sc.ScratchBookId == 3993 || sc.ScratchBookId == 3996) && totalQty > 1)
                            //{
                            //    raisingPotential = 180;
                            //}

                            // hack for beef jerky product quantity les than 3 items itemprice should be $69
                            if (sc.ScratchBookId == 3997  && totalQty < 3)
                            {
                                raisingPotential =  Convert.ToDecimal(GetValueFromWebConfig("BeefJerkyRaisingPotentialAmount", "value"));
                            }
                            else if (sc.ScratchBookId == 3997 && totalQty > 2)
                            {
