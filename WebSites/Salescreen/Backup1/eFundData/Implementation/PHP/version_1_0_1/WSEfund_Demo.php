...
<title>webservice.efundraising.com PHP Client</title>
...
<?php 		
	...
	require_once("phpWebServicesProxy.php");
	// Create a new eFundDataClient object
	$myEfund = new eFundDataClient();

	$isLogin = $myEfund->InitWebServiceConnection('[YOUR_USERNAME]','[YOUR_PASSWORD]');			
	
	if($isLogin['LoginParametersResult'] == 'true') {	
		// Get report passing start datetime and end datetime.
		$myReport = $myEfund->GetLeadsReport("2005-04-10 00:00:00", "2005-04-20 23:59:59");	
		...
		if (array_key_exists('GetLeadsReportResult', $myReport) && $myReport['GetLeadsReportResult'] != "") {
			// Print out the contacts in the array
			$FundraiserList = $myReport['GetLeadsReportResult']['Fundraiser'];
	...
	$myEfund->UnloadWebServiceConnection();		
?>
</body>
</html>
